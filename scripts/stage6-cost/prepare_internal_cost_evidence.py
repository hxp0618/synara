#!/usr/bin/env python3
"""Materialize, validate and atomically publish one Stage 6 internal usage/cost bundle."""
...
from validate_internal_cost_evidence import (  # noqa: E402
    EVIDENCE_IDS,
    MANIFEST_SCHEMA,
    MAX_EVIDENCE_BYTES,
    MAX_MANIFEST_BYTES,
...
DRAFT_SCHEMA = "synara.stage6-internal-cost-evidence-draft.v1"
DRAFT_FIELDS = {
    "schemaVersion",
    "candidate",
    "period",
...
class CostPreparationError(Exception):
    pass


def relative_path(value: Any, label: str) -> pathlib.PurePosixPath:
    if not isinstance(value, str) or not value or "\\" in value or "\x00" in value:
        raise CostPreparationError(f"{label} must be a non-empty POSIX relative path")
    path = pathlib.PurePosixPath(value)
    if path.is_absolute() or not path.parts or any(part in {"", ".", ".."} for part in path.parts):
        raise CostPreparationError(f"{label} must be traversal-free and relative")
    return path
...
def resolve_input(root: pathlib.Path, value: Any, label: str, maximum_bytes: int) -> pathlib.Path:
    ...
    while current != root:
        if current.is_symlink():
            raise CostPreparationError(f"{label} must not traverse a symlink")
...
def resolve_output(root: pathlib.Path, value: Any, label: str) -> pathlib.Path:
    ...
    if not parent.is_dir() or output.exists() or output.is_symlink():
        raise CostPreparationError(f"{label} must be a new path below an existing directory")
    return output
...
def path_reference(root: pathlib.Path, value: Any, label: str) -> dict[str, str]:
    ...
    except (ImmutableEvidenceIOError, CostEvidenceError) as error:
        raise CostPreparationError(str(error)) from error
    return {
...
def materialize_manifest(draft: dict[str, Any], root: pathlib.Path) -> dict[str, Any]:
    ...
    if set(prepared) != DRAFT_FIELDS:
        raise CostPreparationError("draft fields do not match the internal-cost draft schema")
    if prepared.get("schemaVersion") != DRAFT_SCHEMA:
        raise CostPreparationError(f"draft schemaVersion must be {DRAFT_SCHEMA}")
    ...
    if not isinstance(evidence, list) or len(evidence) != len(EVIDENCE_IDS):
        raise CostPreparationError("evidence must contain the four required path references")
    materialized: list[dict[str, str]] = []
    for index, raw in enumerate(evidence):
        try:
            item = exact_keys(raw, {"id", "path"}, f"evidence[{index}]")
        except CostEvidenceError as error:
            raise CostPreparationError(str(error)) from error
        evidence_id = item["id"]
        if not isinstance(evidence_id, str):
            raise CostPreparationError(f"evidence[{index}].id must be a string")
        reference = path_reference(root, item["path"], f"evidence[{index}].path")
        materialized.append({"id": evidence_id, **reference})
    prepared["evidence"] = materialized
    return prepared
...
def validate_evidence_root(value: str) -> pathlib.Path:
    ...
    if supplied.is_symlink():
        raise CostPreparationError("evidence root must be a non-symlink directory")
    try:
        root = supplied.resolve(strict=True)
    except OSError as error:
        raise CostPreparationError("evidence root does not exist") from error
    if not root.is_dir():
        raise CostPreparationError("evidence root must be a directory")
    return root
...
def prepare(args: argparse.Namespace) -> tuple[pathlib.Path, pathlib.Path, dict[str, Any]]:
    ...
    draft_path = resolve_input(root, args.draft, "internal-cost draft", MAX_MANIFEST_BYTES)
    try:
        draft_bytes = read_stable_regular_file(
            draft_path,
            label="internal-cost draft",
            maximum_bytes=MAX_MANIFEST_BYTES,
        )
        scan_for_secret_material(draft_bytes, "internal-cost draft")
        ...
        raise CostPreparationError(
            "internal-cost draft must contain safe valid UTF-8 JSON"
        ) from error
    if not isinstance(draft, dict):
        raise CostPreparationError("internal-cost draft must be an object")
    manifest = materialize_manifest(draft, root)
    manifest_data = encode_json(manifest)
    if len(manifest_data) > MAX_MANIFEST_BYTES:
        raise CostPreparationError("materialized internal-cost manifest exceeds the size limit")
    ...
    if len(output_paths) != 4 or draft_path in output_paths:
        raise CostPreparationError("draft, manifest and receipt output paths must be distinct")
    for output in output_paths:
        if output.exists() or output.is_symlink():
            raise CostPreparationError(f"output already exists: {output.name}")

    temporary_path: pathlib.Path | None = None
    try:
        with tempfile.NamedTemporaryFile(
            mode="wb",
            prefix=".stage6-internal-cost-",
            suffix=".json",
            dir=root,
            delete=False,
        ) as temporary:
    ...
    except (OSError, CostEvidenceError, ImmutableEvidenceIOError) as error:
        raise CostPreparationError(str(error)) from error
    finally:
...
def main() -> int:
    ...
    except CostPreparationError as error:
        parser.exit(2, f"internal cost evidence preparation failed: {error}\n")
    print(
