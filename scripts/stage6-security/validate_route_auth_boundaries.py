...
ROUTE_RE = re.compile(r'^\s*mux\.(Handle|HandleFunc)\("([A-Z]+) ([^"]+)", (.+)\)$')
...
def classify_route(route: str, registration: str, handler: str) -> str:
    identity = route
    if identity in PUBLIC_ROUTES:
        if registration != "HandleFunc" or "server.require" in handler:
            raise RouteBoundaryError(f"public route {identity} has an unexpected wrapper")
        return "public"
    if identity in PLATFORM_SIGNED_ROUTES:
        if registration != "HandleFunc" or handler != "server.publishPlatformRoutingAuthority":
    ...
    if identity in WORKER_REGISTRATION_ROUTES:
        if registration != "HandleFunc" or handler != "server.registerWorker":
    ...
    if identity in ARTIFACT_TOKEN_ROUTES:
        ...
        if registration != "HandleFunc" or handler != expected:
    ...
    if path.startswith("/v1/workers/"):
        if registration != "Handle" or not handler.startswith("server.requireWorker("):
            raise RouteBoundaryError(f"Worker route {identity} is missing requireWorker")
        return "worker"
    if path.startswith("/scim/v2/"):
        if registration != "Handle" or not handler.startswith("server.requireServiceAccount("):
...
def validate(source_path: pathlib.Path) -> dict[str, object]:
    ...
    for line_number, line in enumerate(source.splitlines(), start=1):
        if not line.lstrip().startswith("mux.Handle"):
            continue
        ...
        registration, method, path, handler = match.groups()
        ...
        boundary = classify_route(identity, registration, handler)
    ...
    if not tenant_routes or any(routes[identity] != "login-session" for identity in tenant_routes):
        raise RouteBoundaryError("one or more explicit Tenant routes are outside requireAuth")
    return {
