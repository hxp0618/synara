#!/usr/bin/env python3
"""Materialize, validate and atomically publish one Stage 6 SLO evidence bundle."""
...
from validate_slo_evidence import (  # noqa: E402
    ASSESSMENT,
    EVIDENCE_FIELDS,
    MANIFEST_SCHEMA,
...
DRAFT_SCHEMA = "synara.slo-window-evidence-draft.v1"
MANIFEST_FIELDS = {
    "schemaVersion",
    "windowId",
    "releaseCommit",
...
class SLOPreparationError(Exception):
    pass


def relative_path(value: Any, label: str) -> pathlib.PurePosixPath:
    if not isinstance(value, str) or not value or "\\" in value or "\x00" in value:
        raise SLOPreparationError(f"{label} must be a non-empty POSIX relative path")
    path = pathlib.PurePosixPath(value)
    if path.is_absolute() or not path.parts or any(part in {"", ".", ".."} for part in path.parts):
        raise SLOPreparationError(f"{label} must be traversal-free and relative")
    return path
...
def resolve_input(root: pathlib.Path, value: Any, label: str, maximum_bytes: int) -> pathlib.Path:
    ...
    while current != root:
        if current.is_symlink():
            raise SLOPreparationError(f"{label} must not traverse a symlink")
...
def resolve_output(root: pathlib.Path, value: Any, label: str) -> pathlib.Path:
    ...
    if not parent.is_dir() or output.exists() or output.is_symlink():
        raise SLOPreparationError(f"{label} must be a new path below an existing directory")
    return output
...
def path_reference(root: pathlib.Path, value: Any, label: str) -> dict[str, str]:
    ...
    except (ImmutableEvidenceIOError, SLOEvidenceError) as error:
        raise SLOPreparationError(str(error)) from error
    return {
...
def materialize_manifest(draft: dict[str, Any], root: pathlib.Path) -> dict[str, Any]:
    ...
    if set(prepared) != MANIFEST_FIELDS:
        raise SLOPreparationError("draft fields do not match the SLO draft schema")
    if prepared.get("schemaVersion") != DRAFT_SCHEMA:
        raise SLOPreparationError(f"draft schemaVersion must be {DRAFT_SCHEMA}")
    prepared["schemaVersion"] = MANIFEST_SCHEMA
    try:
        evidence = require_mapping(prepared.get("evidence"), "evidence")
    except SLOEvidenceError as error:
        raise SLOPreparationError(str(error)) from error
    if set(evidence) != set(EVIDENCE_FIELDS):
        raise SLOPreparationError("evidence fields do not match the SLO draft schema")
    prepared["evidence"] = {
...
def validate_evidence_root(value: str) -> pathlib.Path:
    ...
    if supplied.is_symlink():
        raise SLOPreparationError("evidence root must be a non-symlink directory")
    try:
        root = supplied.resolve(strict=True)
    except OSError as error:
        raise SLOPreparationError("evidence root does not exist") from error
    if not root.is_dir():
        raise SLOPreparationError("evidence root must be a directory")
    return root
...
def prepare(args: argparse.Namespace) -> tuple[pathlib.Path, pathlib.Path, dict[str, Any]]:
    ...
    draft_path = resolve_input(root, args.draft, "SLO draft", MAX_MANIFEST_BYTES)
    try:
        ...
        scan_for_secret_material(draft_bytes, "SLO draft")
        ...
        raise SLOPreparationError("SLO draft must contain safe valid UTF-8 JSON") from error
    if not isinstance(draft, dict):
        raise SLOPreparationError("SLO draft must be an object")
    manifest = materialize_manifest(draft, root)
    manifest_data = encode_json(manifest)
    if len(manifest_data) > MAX_MANIFEST_BYTES:
        raise SLOPreparationError("materialized SLO manifest exceeds the size limit")
    ...
    if len(output_paths) != 4 or draft_path in output_paths:
        raise SLOPreparationError("draft, manifest and receipt output paths must be distinct")
    for output in output_paths:
        if output.exists() or output.is_symlink():
            raise SLOPreparationError(f"output already exists: {output.name}")
    ...
    except (OSError, SLOEvidenceError, ImmutableEvidenceIOError) as error:
        raise SLOPreparationError(str(error)) from error
    finally:
...
def main() -> int:
    ...
    except SLOPreparationError as error:
        parser.exit(2, f"SLO evidence preparation failed: {error}\n")
    print(
