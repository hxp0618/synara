...
PREPARE_SCRIPT = DIRECTORY / "prepare_slo_evidence.py"
PREPARE_SPEC = importlib.util.spec_from_file_location("prepare_slo_evidence", PREPARE_SCRIPT)
assert PREPARE_SPEC is not None and PREPARE_SPEC.loader is not None
PREPARE = importlib.util.module_from_spec(PREPARE_SPEC)
PREPARE_SPEC.loader.exec_module(PREPARE)

FIXTURE_SCRIPT = DIRECTORY / "test_validate_slo_evidence.py"
FIXTURE_SPEC = importlib.util.spec_from_file_location("slo_validator_fixture", FIXTURE_SCRIPT)
assert FIXTURE_SPEC is not None and FIXTURE_SPEC.loader is not None
FIXTURE = importlib.util.module_from_spec(FIXTURE_SPEC)
FIXTURE_SPEC.loader.exec_module(FIXTURE)


class PrepareSLOEvidenceTest(unittest.TestCase):
    def setUp(self) -> None:
        self.fixture = FIXTURE.ValidateSLOEvidenceTest(
            "test_validates_complete_window_without_declaring_slo_passed"
        )
        self.fixture.setUp()
        ...
        self.draft = self.root / "slo-draft.json"
        self.manifest = self.root / "slo-manifest.json"
        self.receipt = self.root / "slo-receipt.json"
        self.write_draft()
    ...
    def test_materializes_hashes_and_publishes_validated_outputs(self) -> None:
        ...
        self.assertEqual(
            set(manifest["evidence"]["prometheusQueryEvidence"]), {"path", "sha256"}
        )
    ...
    def test_measured_miss_is_preserved_as_ineligible_receipt(self) -> None:
    ...
    def test_secret_evidence_publishes_nothing_without_echoing_secret(self) -> None:
        secret = "Authorization: Bearer slo-window-secret"
        unsafe = self.evidence / "unsafe-prometheus.json"
        unsafe.write_text(secret + "\n", encoding="utf-8")
        self.payload["evidence"]["prometheusQueryEvidence"] = unsafe.relative_to(
            self.root
        ).as_posix()
        self.write_draft()
        result = subprocess.run(self.command(), check=False, capture_output=True, text=True)
    ...
    def test_rejects_symlinked_evidence_and_traversal_output(self) -> None:
        original = self.root / self.payload["evidence"]["externalProbeEvidence"]
        link = self.evidence / "linked-probe.json"
        link.symlink_to(original.name)
        self.payload["evidence"]["externalProbeEvidence"] = link.relative_to(
            self.root
        ).as_posix()
        self.write_draft()
        result = subprocess.run(self.command(), check=False, capture_output=True, text=True)
        ...
        self.payload["evidence"]["externalProbeEvidence"] = original.relative_to(
            self.root
        ).as_posix()
        self.write_draft()
    ...
    def test_receipt_publication_failure_rolls_back_manifest_pair(self) -> None:
        ...
        with mock.patch.object(
            ...
            with self.assertRaises(PREPARE.SLOPreparationError):
