...
PREPARE_SCRIPT = DIRECTORY / "prepare_capacity_evidence.py"
PREPARE_SPEC = importlib.util.spec_from_file_location(
    "prepare_capacity_evidence", PREPARE_SCRIPT
)
assert PREPARE_SPEC is not None and PREPARE_SPEC.loader is not None
PREPARE = importlib.util.module_from_spec(PREPARE_SPEC)
PREPARE_SPEC.loader.exec_module(PREPARE)

FIXTURE_SCRIPT = DIRECTORY / "test_validate_capacity_evidence.py"
FIXTURE_SPEC = importlib.util.spec_from_file_location(
    "capacity_validator_fixture", FIXTURE_SCRIPT
)
...
class PrepareCapacityEvidenceTest(unittest.TestCase):
    def setUp(self) -> None:
        self.fixture = FIXTURE.ValidateCapacityEvidenceTest(
            "test_validates_headroom_slo_and_non_pass_assessment"
        )
        self.fixture.setUp()
        ...
        self.draft = self.root / "capacity-draft.json"
        self.manifest = self.root / "capacity-manifest.json"
        self.receipt = self.root / "capacity-receipt.json"
        self.write_draft()
    ...
    def args(self) -> Namespace:
        return Namespace(
            ...
            validated_at="2026-07-30T01:00:00Z",
        )
    ...
    def test_materializes_hashes_and_publishes_validated_outputs(self) -> None:
        ...
        self.assertIn("eligibleForHumanGateReview=false", result.stdout)
        manifest = json.loads(self.manifest.read_text(encoding="utf-8"))
        receipt = json.loads(self.receipt.read_text(encoding="utf-8"))
        self.assertEqual(manifest["schemaVersion"], PREPARE.MANIFEST_SCHEMA)
        self.assertEqual(
            set(manifest["evidence"]["workloadGeneratorEvidence"]),
            {"path", "sha256"},
        )
    ...
    def test_capacity_miss_is_preserved_as_ineligible_receipt(self) -> None:
    ...
    def test_secret_evidence_publishes_nothing_without_echoing_secret(self) -> None:
        secret = "Authorization: Bearer capacity-preparer-secret"
        unsafe = self.evidence / "unsafe-capacity-log.json"
        unsafe.write_text(secret + "\n", encoding="utf-8")
        self.payload["evidence"]["applicationLogEvidence"] = unsafe.relative_to(
            self.root
        ).as_posix()
        self.write_draft()
        result = subprocess.run(self.command(), check=False, capture_output=True, text=True)
    ...
    def test_rejects_symlinked_evidence_and_traversal_output(self) -> None:
        original = self.root / self.payload["evidence"]["externalCanaryEvidence"]
        link = self.evidence / "linked-canary.json"
        link.symlink_to(original.name)
        self.payload["evidence"]["externalCanaryEvidence"] = link.relative_to(
            self.root
        ).as_posix()
        self.write_draft()
        result = subprocess.run(self.command(), check=False, capture_output=True, text=True)
        ...
        self.payload["evidence"]["externalCanaryEvidence"] = original.relative_to(
            self.root
        ).as_posix()
        self.write_draft()
    ...
    def test_receipt_publication_failure_rolls_back_manifest_pair(self) -> None:
        ...
        with mock.patch.object(
            ...
            with self.assertRaises(PREPARE.CapacityPreparationError):
