#!/usr/bin/env python3
"""Materialize, validate and atomically publish one Stage 6 capacity bundle."""
...
DRAFT_SCHEMA = "synara.capacity-soak-evidence-draft.v1"
MANIFEST_FIELDS = {
    "schemaVersion",
    "runId",
    "releaseCommit",
...
def relative_path(value: Any, label: str) -> pathlib.PurePosixPath:
    ...
    if path.is_absolute() or not path.parts or any(
        ...
        raise CapacityPreparationError(f"{label} must be traversal-free and relative")
    return path
...
def resolve_input(
    ...
    while current != root:
        if current.is_symlink():
            raise CapacityPreparationError(f"{label} must not traverse a symlink")
...
def path_reference(root: pathlib.Path, value: Any, label: str) -> dict[str, str]:
    ...
    except (ImmutableEvidenceIOError, CapacityEvidenceError) as error:
        raise CapacityPreparationError(str(error)) from error
    return {
...
def materialize_manifest(draft: dict[str, Any], root: pathlib.Path) -> dict[str, Any]:
    ...
    if set(prepared) != MANIFEST_FIELDS:
        raise CapacityPreparationError(
            "draft fields do not match the capacity draft schema"
        )
    if prepared.get("schemaVersion") != DRAFT_SCHEMA:
        raise CapacityPreparationError(f"draft schemaVersion must be {DRAFT_SCHEMA}")
    ...
    if set(evidence) != set(EVIDENCE_FIELDS):
        raise CapacityPreparationError(
            "evidence fields do not match the capacity draft schema"
        )
    prepared["evidence"] = {
...
def validate_evidence_root(value: str) -> pathlib.Path:
    ...
    if supplied.is_symlink():
        raise CapacityPreparationError("evidence root must be a non-symlink directory")
    try:
        root = supplied.resolve(strict=True)
    except OSError as error:
        raise CapacityPreparationError("evidence root does not exist") from error
    if not root.is_dir():
        raise CapacityPreparationError("evidence root must be a directory")
    return root
...
def prepare(
    ...
    draft_path = resolve_input(root, args.draft, "capacity draft", MAX_MANIFEST_BYTES)
    try:
        draft_bytes = read_stable_regular_file(
            draft_path,
            label="capacity draft",
            maximum_bytes=MAX_MANIFEST_BYTES,
        )
        scan_for_secret_material(draft_bytes, "capacity draft")
        draft = json.loads(
            draft_bytes,
        ...
        raise CapacityPreparationError(
            "capacity draft must contain safe valid UTF-8 JSON"
        ) from error
    if not isinstance(draft, dict):
        raise CapacityPreparationError("capacity draft must be an object")
    manifest = materialize_manifest(draft, root)
    manifest_data = encode_json(manifest)
    if len(manifest_data) > MAX_MANIFEST_BYTES:
        raise CapacityPreparationError(
            "materialized capacity manifest exceeds the size limit"
        )
    ...
    for output in output_paths:
        if output.exists() or output.is_symlink():
            raise CapacityPreparationError(f"output already exists: {output.name}")

    temporary_path: pathlib.Path | None = None
    try:
        with tempfile.NamedTemporaryFile(
            mode="wb",
            prefix=".stage6-capacity-",
            suffix=".json",
            dir=root,
            delete=False,
        ) as temporary:
    ...
    except (OSError, CapacityEvidenceError, ImmutableEvidenceIOError) as error:
        raise CapacityPreparationError(str(error)) from error
    finally:
...
def main() -> int:
    ...
    except CapacityPreparationError as error:
        parser.exit(2, f"capacity evidence preparation failed: {error}\n")
    print(
