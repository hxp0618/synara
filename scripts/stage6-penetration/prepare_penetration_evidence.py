#!/usr/bin/env python3
"""Materialize, validate and atomically publish one Stage 6 penetration bundle."""
...
from validate_penetration_evidence import (  # noqa: E402
    ASSESSMENT,
    EVIDENCE_FIELDS,
    MANIFEST_SCHEMA,
    MAX_EVIDENCE_FILE_BYTES,
    MAX_MANIFEST_BYTES,
    PenetrationEvidenceError,
...
DRAFT_SCHEMA = "synara.third-party-penetration-evidence-draft.v1"
MANIFEST_FIELDS = {
    "schemaVersion",
    "engagementId",
    "releaseCommit",
...
class PenetrationPreparationError(Exception):
    pass


def relative_path(value: Any, label: str) -> pathlib.PurePosixPath:
    if not isinstance(value, str) or not value or "\\" in value or "\x00" in value:
        raise PenetrationPreparationError(
...
def resolve_input(
    ...
    while current != root:
        if current.is_symlink():
            raise PenetrationPreparationError(f"{label} must not traverse a symlink")
...
def path_reference(root: pathlib.Path, value: Any, label: str) -> dict[str, str]:
    ...
    except (ImmutableEvidenceIOError, PenetrationEvidenceError) as error:
        raise PenetrationPreparationError(str(error)) from error
    return {
...
def materialize_manifest(draft: dict[str, Any], root: pathlib.Path) -> dict[str, Any]:
    ...
    if set(prepared) != MANIFEST_FIELDS:
        raise PenetrationPreparationError(
            "draft fields do not match the penetration draft schema"
        )
    ...
    except PenetrationEvidenceError as error:
        raise PenetrationPreparationError(str(error)) from error
    if set(evidence) != set(EVIDENCE_FIELDS):
        raise PenetrationPreparationError(
            "evidence fields do not match the penetration draft schema"
        )
    prepared["evidence"] = {
...
def validate_evidence_root(value: str) -> pathlib.Path:
    ...
    try:
        ...
        raise PenetrationPreparationError("evidence root does not exist") from error
    if not root.is_dir():
        raise PenetrationPreparationError("evidence root must be a directory")
    return root
...
def prepare(
    ...
    try:
        draft_bytes = read_stable_regular_file(
            draft_path,
            label="penetration draft",
            maximum_bytes=MAX_MANIFEST_BYTES,
        )
        scan_for_secret_material(draft_bytes, "penetration draft")
        draft = json.loads(
            draft_bytes,
        ...
        raise PenetrationPreparationError(
            "penetration draft must contain safe valid UTF-8 JSON"
        ) from error
    if not isinstance(draft, dict):
        raise PenetrationPreparationError("penetration draft must be an object")
    manifest = materialize_manifest(draft, root)
    manifest_data = encode_json(manifest)
    if len(manifest_data) > MAX_MANIFEST_BYTES:
        raise PenetrationPreparationError(
            "materialized penetration manifest exceeds the size limit"
        )
    ...
    except (OSError, PenetrationEvidenceError, ImmutableEvidenceIOError) as error:
        raise PenetrationPreparationError(str(error)) from error
    finally:
...
def main() -> int:
    ...
    except PenetrationPreparationError as error:
        parser.exit(2, f"penetration evidence preparation failed: {error}\n")
    print(
