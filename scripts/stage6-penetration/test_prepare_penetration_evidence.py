...
PREPARE_SCRIPT = DIRECTORY / "prepare_penetration_evidence.py"
PREPARE_SPEC = importlib.util.spec_from_file_location(
    "prepare_penetration_evidence", PREPARE_SCRIPT
)
assert PREPARE_SPEC is not None and PREPARE_SPEC.loader is not None
PREPARE = importlib.util.module_from_spec(PREPARE_SPEC)
PREPARE_SPEC.loader.exec_module(PREPARE)

FIXTURE_SCRIPT = DIRECTORY / "test_validate_penetration_evidence.py"
FIXTURE_SPEC = importlib.util.spec_from_file_location(
    "penetration_validator_fixture", FIXTURE_SCRIPT
)
...
class PreparePenetrationEvidenceTest(unittest.TestCase):
    def setUp(self) -> None:
        self.fixture = FIXTURE.ValidatePenetrationEvidenceTest(
            "test_validates_complete_bundle_without_declaring_penetration_passed"
        )
        self.fixture.setUp()
        ...
        self.draft = self.root / "penetration-draft.json"
        self.manifest = self.root / "penetration-manifest.json"
        self.receipt = self.root / "penetration-receipt.json"
        self.write_draft()
    ...
    def args(self) -> Namespace:
        return Namespace(
            ...
            validated_at="2026-07-30T05:00:00Z",
        )
    ...
    def test_materializes_hashes_and_publishes_validated_outputs(self) -> None:
        ...
        self.assertEqual(
            set(manifest["evidence"]["finalReportEvidence"]),
            {"path", "sha256"},
        )
    ...
    def test_open_high_is_preserved_as_ineligible_receipt(self) -> None:
        self.payload["findings"] = [
            {
                "findingId": "PEN-PREP-001",
                "severity": "high",
        ...
        self.payload["scopeCoverage"]["crossTenantAuthorization"][
            "result"
        ] = "findings-recorded"
        self.write_draft()
        result = subprocess.run(
    ...
    def test_secret_evidence_publishes_nothing_without_echoing_secret(self) -> None:
        secret = "Authorization: Bearer penetration-report-secret"
        unsafe = self.evidence / "unsafe-final-report.json"
        unsafe.write_text(secret + "\n", encoding="utf-8")
        self.payload["evidence"]["finalReportEvidence"] = unsafe.relative_to(
            self.root
        ).as_posix()
        self.write_draft()
        result = subprocess.run(
    ...
    def test_rejects_symlinked_evidence_and_traversal_output(self) -> None:
        original = self.root / self.payload["evidence"]["executionEvidence"]
        link = self.evidence / "linked-execution.json"
        link.symlink_to(original.name)
        self.payload["evidence"]["executionEvidence"] = link.relative_to(
            self.root
        ).as_posix()
        self.write_draft()
        result = subprocess.run(
        ...
        self.payload["evidence"]["executionEvidence"] = original.relative_to(
            self.root
        ).as_posix()
        self.write_draft()
    ...
    def test_receipt_publication_failure_rolls_back_manifest_pair(self) -> None:
        ...
        with mock.patch.object(
            ...
            with self.assertRaises(PREPARE.PenetrationPreparationError):
