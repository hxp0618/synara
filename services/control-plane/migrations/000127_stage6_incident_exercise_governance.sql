...
CREATE TABLE stage6_incident_exercises (
  id UUID PRIMARY KEY,
  ...
  exercise_mode TEXT NOT NULL CHECK (exercise_mode = 'live-public-communication'),
  severity TEXT NOT NULL CHECK (severity IN ('SEV-0', 'SEV-1', 'SEV-2')),
  service_origin TEXT NOT NULL CHECK (length(service_origin) BETWEEN 8 AND 512),
  status_page_origin TEXT NOT NULL CHECK (length(status_page_origin) BETWEEN 8 AND 512),
  started_at TIMESTAMPTZ NOT NULL,
  completed_at TIMESTAMPTZ NOT NULL,
  validated_at TIMESTAMPTZ NOT NULL,
  independent_status_page_declared BOOLEAN NOT NULL,
  role_separation_complete BOOLEAN NOT NULL,
  paging_exercise_complete BOOLEAN NOT NULL,
  status_page_components_complete BOOLEAN NOT NULL,
  public_timeline_within_targets BOOLEAN NOT NULL,
  subscriber_delivery_complete BOOLEAN NOT NULL,
  recovery_verification_complete BOOLEAN NOT NULL,
...
CREATE INDEX idx_stage6_incident_exercises_candidate
  ON stage6_incident_exercises (candidate_record_id, created_at DESC, id);

CREATE TABLE stage6_incident_exercise_approvals (
  id UUID PRIMARY KEY,
  incident_exercise_id UUID NOT NULL REFERENCES stage6_incident_exercises(id) ON DELETE RESTRICT,
  operator_tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE RESTRICT,
  approval_role TEXT NOT NULL CHECK (approval_role IN ('operations', 'communications')),
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE (incident_exercise_id, approval_role),
  UNIQUE (incident_exercise_id, approver_user_id)
);

CREATE INDEX idx_stage6_incident_exercise_approvals_exercise
  ON stage6_incident_exercise_approvals (incident_exercise_id, created_at, id);
...
BEGIN
  ...
  IF NEW.state = 'approved' THEN
    ...
    IF approval_count <> 2 OR NOT NEW.eligible_for_human_gate_review
      ...
      RAISE EXCEPTION 'Stage 6 Incident exercise approval gate is incomplete' USING ERRCODE = '23514';
    END IF;
...
BEGIN
  ...
  IF parent.id IS NULL OR parent.operator_tenant_id <> NEW.operator_tenant_id OR parent.state <> 'recorded'
    ...
    RAISE EXCEPTION 'Invalid Stage 6 Incident exercise approval authority' USING ERRCODE = '23514';
  END IF;
  RETURN NEW;
END;
$$;

CREATE TRIGGER trg_stage6_incident_exercise_approvals_insert
BEFORE INSERT ON stage6_incident_exercise_approvals
FOR EACH ROW EXECUTE FUNCTION enforce_stage6_incident_exercise_approval();

CREATE OR REPLACE FUNCTION reject_stage6_incident_exercise_history_mutation()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'Stage 6 Incident exercise evidence history is immutable' USING ERRCODE = '23514';
END;
$$;

CREATE TRIGGER trg_stage6_incident_exercise_approvals_no_update BEFORE UPDATE ON stage6_incident_exercise_approvals
FOR EACH ROW EXECUTE FUNCTION reject_stage6_incident_exercise_history_mutation();
CREATE TRIGGER trg_stage6_incident_exercise_approvals_no_delete BEFORE DELETE ON stage6_incident_exercise_approvals
FOR EACH ROW EXECUTE FUNCTION reject_stage6_incident_exercise_history_mutation();
CREATE TRIGGER trg_stage6_incident_exercises_no_delete BEFORE DELETE ON stage6_incident_exercises
FOR EACH ROW EXECUTE FUNCTION reject_stage6_incident_exercise_history_mutation();

COMMENT ON TABLE stage6_incident_exercises IS
  'Internal governance over exact paging and public-communication exercise receipts; approval does not authenticate the external Status Page, paging provider, subscriber delivery, signatures, execution, or external approver authority.';
