...
func TestValidateRuntimeEventContractChecksCanonicalPayloadShape(t *testing.T) {
	tests := []struct {
		...
		{name: "turn steered", eventType: "turn.steered", payload: map[string]any{"message": "Continue with the queued instruction"}, valid: true},
		{name: "turn steered requires message", eventType: "turn.steered", payload: map[string]any{}},
		...
		{name: "sensitive approval requested", eventType: "request.opened", payload: map[string]any{"requestId": "approval-sensitive", "requestType": "command_execution_approval", "sensitiveAction": map[string]any{"categories": []any{"credential-access", "network-egress"}, "requiresFreshApproval": true, "allowSessionApproval": false}}, valid: true},
		{name: "sensitive approval categories must be canonical", eventType: "request.opened", payload: map[string]any{"requestId": "approval-sensitive", "requestType": "command_execution_approval", "sensitiveAction": map[string]any{"categories": []any{"network-egress", "credential-access"}, "requiresFreshApproval": true, "allowSessionApproval": false}}},
		{name: "sensitive approval cannot allow session grant", eventType: "request.opened", payload: map[string]any{"requestId": "approval-sensitive", "requestType": "command_execution_approval", "sensitiveAction": map[string]any{"categories": []any{"network-egress"}, "requiresFreshApproval": true, "allowSessionApproval": true}}},
		{name: "sensitive approval resolution", eventType: "request.resolved", payload: map[string]any{"requestId": "approval-sensitive", "requestType": "command_execution_approval", "decision": "accept", "sensitiveAction": map[string]any{"categories": []any{"network-egress"}, "requiresFreshApproval": true, "allowSessionApproval": false}}, valid: true},
		...
		{name: "task updated", eventType: "task.updated", payload: map[string]any{"taskId": "workflow-1", "status": "paused", "isBackgrounded": true, "workflowRunId": "run-1"}, valid: true},
		{name: "task updated rejects unknown status", eventType: "task.updated", payload: map[string]any{"taskId": "workflow-1", "status": "waiting"}},
