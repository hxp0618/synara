...
// Stage6CapacityRun retains an exact capacity/soak validator receipt bound to
// one release candidate. Internal approval does not prove that the external
// environment, telemetry, signatures, or approver authority are authentic.
type Stage6CapacityRun struct {
	...
	RunID                                 uuid.UUID  `gorm:"column:run_id;type:uuid;not null;uniqueIndex"`
	...
	MinimumDurationSeconds                int64      `gorm:"column:minimum_duration_seconds;not null"`
	SampleIntervalSeconds                 int64      `gorm:"column:sample_interval_seconds;not null"`
	ExternalProbeRegions                  int64      `gorm:"column:external_probe_regions;not null"`
	ExternalProbeCoverageRatio            float64    `gorm:"column:external_probe_coverage_ratio;not null"`
	ForecastHeadroomCovered               bool       `gorm:"column:forecast_headroom_covered;not null"`
	PhaseCoverageComplete                 bool       `gorm:"column:phase_coverage_complete;not null"`
	ExerciseCoverageComplete              bool       `gorm:"column:exercise_coverage_complete;not null"`
...
func (Stage6CapacityRun) TableName() string { return "stage6_capacity_runs" }

type Stage6CapacityPhase struct {
	ID               uuid.UUID `gorm:"column:id;type:uuid;primaryKey"`
	CapacityRunID    uuid.UUID `gorm:"column:capacity_run_id;type:uuid;not null"`
	OperatorTenantID uuid.UUID `gorm:"column:operator_tenant_id;type:uuid;not null"`
	Name             string    `gorm:"column:phase_name;not null"`
	StartedAt        time.Time `gorm:"column:started_at;not null"`
	CompletedAt      time.Time `gorm:"column:completed_at;not null"`
	DurationSeconds  int64     `gorm:"column:duration_seconds;not null"`
	LoadMultiplier   float64   `gorm:"column:load_multiplier;not null"`
	CreatedAt        time.Time `gorm:"column:created_at"`
}

func (Stage6CapacityPhase) TableName() string { return "stage6_capacity_phases" }

type Stage6CapacityApproval struct {
	ID                uuid.UUID  `gorm:"column:id;type:uuid;primaryKey"`
	CapacityRunID     uuid.UUID  `gorm:"column:capacity_run_id;type:uuid;not null"`
	OperatorTenantID  uuid.UUID  `gorm:"column:operator_tenant_id;type:uuid;not null"`
...
func (Stage6CapacityApproval) TableName() string { return "stage6_capacity_approvals" }
