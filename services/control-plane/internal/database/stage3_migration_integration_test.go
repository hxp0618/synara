...
func seedStage3MigrationState(t *testing.T, db *gorm.DB) stage3MigrationSeed {
	...
	models := []struct {
		...
		{&persistence.ExecutionTarget{ID: targetID, TenantID: &tenantID, OrganizationID: &organizationID, Kind: "kubernetes", Name: "Migration target", Status: "active", ConfigurationEncrypted: []byte{}, Capabilities: map[string]any{}},
			[]string{"id", "tenant_id", "organization_id", "kind", "name", "status", "configuration_encrypted", "capabilities", "created_at", "updated_at"}},
