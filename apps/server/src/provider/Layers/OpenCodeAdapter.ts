  ...
  return Layer.effect(
    ...
    Effect.gen(function* () {
            ...
            Effect.gen(function* () {
              const emitted = yield* restore(
                emit(context, {
                  ...
                  payload: {
                    requestType: mapPermissionToRequestType(claim.request.request.permission),
                    decision: mapPermissionDecision(input.reply),
                    ...(claim.request.sensitiveAction.requiresFreshApproval
                      ? { sensitiveAction: claim.request.sensitiveAction }
                      : {}),
                    ...(claim.request.requestedDecision
                      ? { requestedDecision: claim.request.requestedDecision }
                      : {}),
                  },
                }),
              ).pipe(Effect.exit);
      ...
      const respondToRequest: OpenCodeAdapterShape["respondToRequest"] = Effect.fn(
        ...
        const reply = toOpenCodePermissionReply(effectiveDecision);
