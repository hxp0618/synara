...
export function syncAuthoritativeProjection(
  ...
  const normalizedProjects = normalizeAuthoritativeProjects(
    projects.filter((project) => deletedProjectIdsById[project.id] === undefined),
    state.projects,
  );
...
export function syncServerReadModel(state: AppState, readModel: OrchestrationReadModel): AppState {
  if (controlPlaneProjectionActive(state) || isStaleSnapshot(state, readModel.snapshotSequence)) {
    return state;
  }
  rememberProjectUiState(state.projects);
  rememberProjectLocalNames(state.projects);
