...
import { useControlPlane, useControlPlanePendingInteractions } from "../controlPlaneContext";
import {
...
function buildThreadTitleSeed(input: {
  ...
  const firstBrowserAnnotation = input.browserAnnotations[0];
  if (firstBrowserAnnotation) {
    return formatBrowserAnnotationLabel(firstBrowserAnnotation);
  }
  const firstTerminalContext = input.terminalContexts[0];
  if (firstTerminalContext) {
    return formatTerminalContextLabel(firstTerminalContext);
...
export default function ChatView({
  ...
  const activePendingUserInputKey = activePendingUserInput
    ? (activePendingUserInput.requestKey ??
  ...
  const activePendingApprovalKey = activePendingApproval
    ? (activePendingApproval.requestKey ??
      pendingRequestInstanceKey(
  ...
  useEffect(() => {
    ...
    const nextRequestKey = activePendingUserInputKey;
    const nextQuestionId = activePendingProgress?.activeQuestion?.id ?? null;
    const questionChanged =
      lastSyncedPendingInputRef.current?.requestKey !== nextRequestKey ||
  ...
  const onSend = async (
    ...
    if (controlPlane.isAuthoritative) {
      ...
      // Built before the try: React Compiler's BuildHIR cannot lower a value block
      // (ternary, conditional spread) inside a try statement, and one bails out this
      // whole component. See chatHotPath.compiler.test.ts.
      const authoritativeTurnRequest = {
      ...
      try {
        const result = await dispatchAuthoritativeTurn(authoritativeTurnRequest);
  ...
  const onRespondToApproval = useCallback(
    async (
      ...
      if (
        ...
        approval.lifecycleGeneration !== lifecycleGeneration ||
        !requestKey ||
        !activeThreadId
      ) {
        return;
      ...
      // Value blocks (`?.`, `??`, ternary, conditional spread) cannot be lowered
      ...
      // bails out this whole component. The catch above swallows every rejection
      // and the try body has no early return, so this is equivalent.
  ...
  const onRespondToUserInput = useCallback(
    async (
      ...
      if (
        ...
        pendingInput.lifecycleGeneration !== lifecycleGeneration ||
        !requestKey ||
        !activeThreadId
      ) {
        return;
  ...
  async function onSubmitPlanFollowUp({
    ...
    setThreadError(threadIdForSend, null);
    if (controlPlane.isAuthoritative) {
      // Built before the try: see the note on `authoritativeTurnRequest` above.
      const queuedAuthoritativeTurnRequest = {
        draftThreadId: threadIdForSend,
      ...
      try {
        await dispatchAuthoritativeTurn(queuedAuthoritativeTurnRequest);
                ...
                <div className="pb-2">
                  <ComposerPendingApprovalPanel
                    ...
                    isResponding={
                      ...
                      respondingRequestKeys.includes(activePendingApprovalKey)
                    }
