...
export function TenantCredentialSettingsSection(props: {
  ...
  const create = async (event: FormEvent) => {
    ...
    try {
      ...
      queryClient.setQueryData<{
        items: ReadonlyArray<ControlPlaneCredential>;
      }>(credentialsQueryKey(props.tenantId), (current) => ({
  ...
  const revoke = async (item: ControlPlaneCredential) => {
    if (
      !confirmAction(`Revoke ${item.name}? Running agents will no longer be able to resolve it.`)
    ) {
      return;
    }
    setRevokeError(null);
  ...
  return (
    <SettingsSection title="Credentials">
            ...
            {props.canManage ? (
              ...
              <>
                <ControlPlaneStatusPill
                  value={scopePolicy.data!.platformCredentialsEnabled ? "enabled" : "disabled"}
                  active={scopePolicy.data!.platformCredentialsEnabled}
                />
                <ControlPlaneStatusPill
                  value={
                    scopePolicy.data!.platformCredentialAutoSelect
                  ...
                  active={scopePolicy.data!.platformCredentialAutoSelect}
                />
              </>
            )}
          </div>
        ...
        <SettingsListRow
          ...
          actions={
            <span className="flex flex-wrap items-center justify-end gap-1.5">
              ...
              {props.canManage && item.revokedAt === null ? (
                <>
          ...
          >
            ...
            {scope === "platform" ? (
              <div className="text-xs text-muted-foreground sm:col-span-2">
                Platform scope requires an enterprise installation, an Enterprise entitlement
                profile, and the explicit policy above. It is still Tenant-owned and never shared
      ...
      {props.canManage && activeCredentials.length > 0 ? (
        <SettingsRow
        ...
        >
          ...
          >
            ...
            <div className={`sm:col-span-2${selectedRotation ? "" : " text-muted-foreground"}`}>
              <Button
