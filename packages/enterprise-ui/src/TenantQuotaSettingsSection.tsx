...
import { controlPlaneClient, type ControlPlaneTenantQuota } from "@synara/control-plane-client";
...
export function quotaQueryKey(tenantId: string) {
...
export function TenantQuotaSettingsSection(props: { tenantId: string; canManage: boolean }) {
  const { Button, SettingsListRow, SettingsSection, StatusPill } = useEnterpriseUiHost();
      ...
      <SettingsListRow
        ...
        actions={
          item.maxConcurrentExecutions === null ? (
            <StatusPill value="unlimited" active={false} />
          ) : (
...
function TenantQuotaForm(props: {
  ...
  const { Button, FormField, InlineError, Input, SettingsRow, formGridClassName } =
    useEnterpriseUiHost();
  ...
  return (
    ...
    >
      <form className={formGridClassName} onSubmit={submit}>
        <FormField label="Concurrent executions">
          <Input
            inputMode="numeric"
        ...
        <FormField label="Artifact storage bytes">
          <Input
            inputMode="numeric"
