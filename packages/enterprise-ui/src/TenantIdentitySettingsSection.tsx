...
import { TenantIdentityGovernanceSettings } from "./TenantIdentityGovernanceSettings";

const TENANT_ROLES = ["member", "auditor", "cost_admin", "security_admin", "admin"] as const;
const ORGANIZATION_ROLES = ["viewer", "member", "agent_operator", "admin", "owner"] as const;

export function identityConnectionsQueryKey(tenantId: string) {
  return ["control-plane", "tenants", tenantId, "identity-connections"] as const;
}

export function identityGroupMappingsQueryKey(tenantId: string, connectionId: string) {
  return [
    "control-plane",
    "tenants",
    tenantId,
...
export function TenantIdentitySettingsSection(props: {
  ...
  const { InlineError, SettingsListRow, SettingsRow, SettingsSection } = useEnterpriseUiHost();
  const queryClient = useQueryClient();
  const connections = useQuery({
    queryKey: identityConnectionsQueryKey(props.tenantId),
  ...
  const onCreated = (connection: ControlPlaneIdentityConnection) => {
    queryClient.setQueryData<{ items: ReadonlyArray<ControlPlaneIdentityConnection> }>(
      identityConnectionsQueryKey(props.tenantId),
          ...
          {props.connection.kind === "saml" ? (
            <span className="block">
              ...
              <a
                ...
                href={resolveControlPlaneHttpUrl(
                  `/v1/auth/sso/${encodeURIComponent(props.connection.id)}/metadata`,
                )}
                rel="noreferrer"
                target="_blank"
              >
...
function IdentityGroupMappingsForm(props: {
  ...
  const mappings = useQuery({
    queryKey: identityGroupMappingsQueryKey(props.tenantId, props.connectionId),
  ...
  const replace = useMutation({
    ...
    onSuccess: (next) =>
      queryClient.setQueryData(
        identityGroupMappingsQueryKey(props.tenantId, props.connectionId),
        next,
      ),
  });
