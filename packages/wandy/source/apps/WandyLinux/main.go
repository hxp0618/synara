...
func runPython(request linuxRequest) (*linuxResponse, error) {
	...
	tempDir, err := os.MkdirTemp("", "wandy-linux-*")
	if err != nil {
		return nil, err
	}
...
func runCLI(args []string, stdout io.Writer) error {
	...
	case "doctor":
		fmt.Fprintln(stdout, "Linux runtime: AT-SPI2 and GDK run against the signed-in desktop user's accessibility session. When Codex starts without XDG_RUNTIME_DIR, DBUS_SESSION_BUS_ADDRESS, or display variables, wandy tries to discover the same user's session from /run/user/<uid> and desktop processes.")
		return nil
	case "list-apps":
