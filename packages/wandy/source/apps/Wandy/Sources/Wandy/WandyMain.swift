...
enum WandyMain {
    ...
    static func main() {
        do {
            try run()
        } catch let error as WandyCLIError {
    ...
    private static func shouldSuppressOnboarding() -> Bool {
        switch ProcessInfo.processInfo.environment["WANDY_SUPPRESS_ONBOARDING"]?.lowercased() {
        case "1", "true", "yes", "on":
            return true
        default:
            return false
