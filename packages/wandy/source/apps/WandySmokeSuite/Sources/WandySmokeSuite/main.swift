...
@main
enum WandySmokeSuite {
    static func main() throws {
        let productsDirectory = try locateProductsDirectory()
        let fixtureURL = productsDirectory.appendingPathComponent("WandyFixture")
        let serverURL = productsDirectory.appendingPathComponent("Wandy")
        let appName = "WandyFixture"
    ...
    private static func runCursorIdleSmoke(serverURL: URL, appName: String) throws {
        ...
        environment["WANDY_VISUAL_CURSOR"] = "1"
        environment["WANDY_VISUAL_CURSOR_OBSERVATION_FILE"] = observationURL.path
    ...
    private static func smokeServerEnvironment() -> [String: String] {
        ...
        environment["WANDY_DISABLE_APP_AGENT_PROXY"] = "1"
        return environment
    }

    private static func smokeFixtureEnvironment() -> [String: String] {
        var environment = ProcessInfo.processInfo.environment
        environment["WANDY_FIXTURE_HEADLESS"] = "1"
        return environment
    }
    ...
    private static func cursorObservationFileURL() -> URL {
        URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("wandy-smoke", isDirectory: true)
