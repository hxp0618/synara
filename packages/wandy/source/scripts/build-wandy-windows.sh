...
print_help() {
  cat <<'EOF'
Usage:
  scripts/build-wandy-windows.sh [options]
...
version="$(node -e "console.log(require('./plugins/wandy/.codex-plugin/plugin.json').version)")"
module_dir="${repo_root}/apps/WandyWindows"
...
(
  ...
  GOOS=windows GOARCH="${arch}" CGO_ENABLED=0 go build \
    ...
    -o "${output_dir}/wandy.exe" \
    .
)

echo "Built ${configuration} Windows runtime: ${output_dir}/wandy.exe"
