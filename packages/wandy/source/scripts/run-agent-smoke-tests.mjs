...
const selectedAgents = new Set(
  (args.get("agents") ?? process.env.WANDY_AGENT_AGENTS ?? "claude,codex")
    .split(",")
    .map((agent) => agent.trim())
    .filter(Boolean),
);
const json = args.get("json") === "true";
const command = args.get("command") ?? process.env.WANDY_AGENT_COMMAND ?? "wandy";
const timeoutMs = Number(args.get("timeout-ms") ?? process.env.WANDY_AGENT_TIMEOUT_MS ?? 120000);
const maxClaudeBudget =
  args.get("claude-budget-usd") ?? process.env.WANDY_CLAUDE_BUDGET_USD ?? "2.00";
const hermesProvider = args.get("hermes-provider") ?? process.env.WANDY_HERMES_PROVIDER;
const hermesModel = args.get("hermes-model") ?? process.env.WANDY_HERMES_MODEL;
const hermesMaxTurns = args.get("hermes-max-turns") ?? process.env.WANDY_HERMES_MAX_TURNS ?? "12";
...
function fixtureExecutablePath() {
  for (const candidate of [".build/release/WandyFixture", ".build/debug/WandyFixture"]) {
    if (existsSync(candidate)) return candidate;
  }

  const build = spawnSync("swift", ["build", "-c", "release", "--product", "WandyFixture"], {
    cwd: process.cwd(),
    stdio: "inherit",
  });
  if (build.status !== 0) {
    throw new Error("Failed to build WandyFixture.");
  }
  if (existsSync(".build/release/WandyFixture")) return ".build/release/WandyFixture";
  throw new Error("WandyFixture executable was not found after build.");
}

function resetFixtureState() {
  rmSync(`${process.env.TMPDIR ?? "/tmp"}/wandy-fixture`, { recursive: true, force: true });
}
...
function callWandy(tool, toolArgs = {}) {
...
async function waitForFixtureReady(fixture) {
  ...
  while (Date.now() < deadline) {
    if (fixture?.fixtureExitCode !== null) {
      throw new Error(
        [
          `WandyFixture exited before readiness with code ${fixture.fixtureExitCode}.`,
    ...
    try {
      const result = await callWandy("get_app_state", { app: "WandyFixture" });
  ...
  throw new Error(
    [
      "WandyFixture did not become visible to wandy.",
...
function makePrompt(expectedValue) {
  if (scenario === "list-apps") {
    return [
      "Use the wandy MCP list_apps tool exactly once before answering.",
  ...
  return [
    "Use only wandy MCP tools.",
    "Call get_app_state for app WandyFixture.",
    ...(scenario === "fixture-full"
      ? [
  ...
  return {
    ...
    codex: {
      ...
      args: [
        ...
        "--json",
        "-c",
        `mcp_servers.wandy.command=${JSON.stringify(command)}`,
        "-c",
        'mcp_servers.wandy.args=["mcp"]',
        prompt,
      ],
      validate: (result) =>
