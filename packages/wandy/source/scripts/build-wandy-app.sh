...
configuration="debug"
arch_mode="native"
codesign_mode="${WANDY_CODESIGN_MODE:-auto}"
codesign_identity="${WANDY_CODESIGN_IDENTITY:-}"
codesign_keychain="${WANDY_CODESIGN_KEYCHAIN:-}"

usage() {
  cat <<'EOF'
Usage: ./scripts/build-wandy-app.sh [debug|release] [--configuration debug|release] [--arch native|arm64|x86_64|universal]

Examples:
  ./scripts/build-wandy-app.sh debug
  ./scripts/build-wandy-app.sh --configuration release --arch universal

Environment:
  WANDY_CODESIGN_MODE=auto|identity|adhoc|none
  WANDY_CODESIGN_IDENTITY="Developer ID Application: Example, Inc. (TEAMID)"
  WANDY_CODESIGN_KEYCHAIN=/path/to/signing.keychain-db
EOF
}
...
if [[ "${codesign_mode}" != "auto" && "${codesign_mode}" != "identity" && "${codesign_mode}" != "adhoc" && "${codesign_mode}" != "none" ]]; then
  echo "Unsupported WANDY_CODESIGN_MODE: ${codesign_mode}" >&2
  exit 1
fi

read_package_version() {
  python3 - "${repo_root}/plugins/wandy/.codex-plugin/plugin.json" <<'PY'
import json
import sys
...
build_binary() {
  ...
  swift build "${args[@]}" --product Wandy >&2
  printf '%s/Wandy\n' "${binary_dir}"
}
  ...
  case "${codesign_mode}" in
    ...
    identity)
      if [[ -z "${codesign_identity}" ]]; then
        echo "WANDY_CODESIGN_IDENTITY is required when WANDY_CODESIGN_MODE=identity" >&2
        exit 1
      fi
      printf '%s\n' "${codesign_identity}"
      return 0
...
codesign_app_bundle() {
  ...
  if ! identity="$(resolve_codesign_identity)"; then
    echo "Skipping codesign for ${app_path} (WANDY_CODESIGN_MODE=none)" >&2
    return
  fi
...
bundle_version="${WANDY_BUNDLE_VERSION:-$(git -C "${repo_root}" rev-list --count HEAD 2>/dev/null || echo 1)}"
release_app_bundle_name="Wandy.app"
development_app_bundle_name="Wandy (Dev).app"
legacy_app_bundle_name="Wandy.app"
bundle_icon_name="Wandy.icns"
icon_master_png="${repo_root}/assets/app-icons/wandy-1024.png"
...
bundled_cursor_reference_source="${repo_root}/../dist/Wandy.app/Contents/Resources/official-software-cursor-window-252.png"

bundle_display_name="Wandy"
bundle_identifier="com.synara.wandy"
app_variant="release"
app_bundle_name="${release_app_bundle_name}"

if [[ "${configuration}" != "release" ]]; then
  bundle_display_name="Wandy (Dev)"
  bundle_identifier="com.synara.wandy.dev"
  app_variant="dev"
...
case "${arch_mode}" in
  native)
    cp "$(build_binary "" "")" "${macos_dir}/Wandy"
    ;;
  arm64)
    cp "$(build_binary "arm64-apple-macosx14.0" ".build/arm64-${configuration}")" "${macos_dir}/Wandy"
    ;;
  x86_64)
    cp "$(build_binary "x86_64-apple-macosx14.0" ".build/x86_64-${configuration}")" "${macos_dir}/Wandy"
    ;;
  universal)
    ...
    lipo -create -output "${macos_dir}/Wandy" "${arm_binary}" "${x86_binary}"
    ;;
esac

chmod +x "${macos_dir}/Wandy"
...
if [[ ! -f "${cursor_reference_source}" && -f "${bundled_cursor_reference_source}" ]]; then
  cursor_reference_source="${bundled_cursor_reference_source}"
fi
...
icon_work_dir="$(mktemp -d "${TMPDIR:-/tmp}/wandy-icon.XXXXXX")"
cleanup() {
...
trap cleanup EXIT
iconset_dir="${icon_work_dir}/Wandy.iconset"
mkdir -p "${iconset_dir}"
