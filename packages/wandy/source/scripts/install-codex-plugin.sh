...
marketplace_name="wandy-local"
plugin_name="wandy"
plugin_source_root="${repo_root}/plugins/${plugin_name}"
plugin_manifest="${plugin_source_root}/.codex-plugin/plugin.json"
macos_build_script="${repo_root}/scripts/build-wandy-app.sh"
linux_build_script="${repo_root}/scripts/build-wandy-linux.sh"
windows_build_script="${repo_root}/scripts/build-wandy-windows.sh"
configuration="debug"
rebuild="false"
...
resolve_app_bundle() {
  ...
  if [[ "${configuration}" == "release" ]]; then
    candidates=("Wandy.app")
  else
    candidates=("Wandy (Dev).app" "Wandy.app")
  fi
...
resolve_native_binary() {
  ...
  case "${platform}" in
    linux)
      if [[ -n "${go_arch}" ]]; then
        candidates+=("${repo_root}/dist/linux/${go_arch}/wandy")
      fi
      ;;
    win32)
      if [[ -n "${go_arch}" ]]; then
        candidates+=("${repo_root}/dist/windows/${go_arch}/wandy.exe")
      fi
      ;;
  esac
...
case "${platform}" in
  darwin)
    ...
    app_binary="${payload_path:+${payload_path}/Contents/MacOS/Wandy}"
