...
cli="${WANDY_E2E_CLI:-${repo_root}/.build/debug/Wandy}"
timeout_seconds="${WANDY_E2E_TIMEOUT_SECONDS:-3}"
disable_app_agent_proxy="${WANDY_E2E_DISABLE_APP_AGENT_PROXY:-1}"

cd "${repo_root}"

if [[ -z "${WANDY_E2E_CLI:-}" ]]; then
  swift build --product Wandy
fi
...
if [[ ! -x "${cli}" ]]; then
  if command -v wandy >/dev/null 2>&1; then
    ...
    echo "Run swift build first, or set WANDY_E2E_CLI=/path/to/wandy." >&2
    exit 1
  fi
fi

tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/wandy-permission-e2e.XXXXXX")"
cleanup() {
  rm -rf "${tmpdir}"
}
trap cleanup EXIT
...
if [[ "${disable_app_agent_proxy}" == "1" || "${disable_app_agent_proxy}" == "true" || "${disable_app_agent_proxy}" == "yes" ]]; then
  ...
  run_cli() {
    WANDY_DISABLE_APP_AGENT_PROXY=1 "${cli}" "$@"
  }
else
