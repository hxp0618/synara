...
import ImageIO
import XCTest
@testable import WandyKit

final class WandyKitTests: XCTestCase {
    func testCLIRecognizesGlobalHelpAndVersionFlags() throws {
        XCTAssertEqual(try parseWandyCLI(arguments: ["-h"]), .help(command: nil))
        XCTAssertEqual(try parseWandyCLI(arguments: ["--help"]), .help(command: nil))
        XCTAssertEqual(try parseWandyCLI(arguments: ["-v"]), .version)
        XCTAssertEqual(try parseWandyCLI(arguments: ["--version"]), .version)
    }

    func testCLIRecognizesCommandSpecificHelp() throws {
        XCTAssertEqual(try parseWandyCLI(arguments: ["help", "snapshot"]), .help(command: "snapshot"))
        XCTAssertEqual(try parseWandyCLI(arguments: ["snapshot", "--help"]), .help(command: "snapshot"))
        XCTAssertEqual(try parseWandyCLI(arguments: ["doctor", "-h"]), .help(command: "doctor"))
        XCTAssertEqual(try parseWandyCLI(arguments: ["call", "--help"]), .help(command: "call"))
    }

    func testCLIRecognizesDoctorJSONOutput() throws {
        XCTAssertEqual(try parseWandyCLI(arguments: ["doctor"]), .doctor(format: .text))
        XCTAssertEqual(try parseWandyCLI(arguments: ["doctor", "--json"]), .doctor(format: .json))
    }

    func testCLIRecognizesSingleToolCallCommand() throws {
        XCTAssertEqual(
            try parseWandyCLI(arguments: ["call", "list_apps"]),
            .call(.single(toolName: "list_apps", argumentsJSON: nil, argumentsFile: nil))
        )

        XCTAssertEqual(
            try parseWandyCLI(arguments: ["call", "get_app_state", "--args", #"{"app":"TextEdit"}"#]),
    ...
    func testCLIRecognizesJSONSequenceCallCommand() throws {
        ...
        XCTAssertEqual(
            try parseWandyCLI(arguments: ["call", "--calls", calls]),
            .call(.sequence(
                callsJSON: calls,
                callsFile: nil,
    ...
    func testCLIRecognizesJSONSequenceCallCommandWithCustomSleep() throws {
        ...
        XCTAssertEqual(
            try parseWandyCLI(arguments: ["call", "--calls", calls, "--sleep", "0.5"]),
    ...
    func testCLIRecognizesTurnEndedNotifyPayload() throws {
        ...
        XCTAssertEqual(try parseWandyCLI(arguments: ["turn-ended"]), .turnEnded(payload: nil))
        XCTAssertEqual(try parseWandyCLI(arguments: ["turn-ended", payload]), .turnEnded(payload: payload))
        XCTAssertEqual(
            try parseWandyCLI(arguments: ["turn-ended", "--previous-notify", #"["/bin/true"]"#, payload]),
            .turnEnded(payload: payload)
        )
    }

    func testCLIRequiresSnapshotArgument() {
        XCTAssertThrowsError(try parseWandyCLI(arguments: ["snapshot"])) { error in
            XCTAssertEqual(
                error as? WandyCLIError,
                WandyCLIError(
    ...
    func testCLIRejectsMixedCallSequenceInputs() {
        XCTAssertThrowsError(try parseWandyCLI(arguments: ["call", "list_apps", "--calls", "[]"])) { error in
            XCTAssertEqual(
                error as? WandyCLIError,
                WandyCLIError(
    ...
    func testCLIRejectsSleepForSingleToolCall() {
        XCTAssertThrowsError(try parseWandyCLI(arguments: ["call", "list_apps", "--sleep", "0.5"])) { error in
            XCTAssertEqual(
                error as? WandyCLIError,
                WandyCLIError(
    ...
    func testCLIRejectsInvalidSequenceSleepValue() {
        XCTAssertThrowsError(try parseWandyCLI(arguments: ["call", "--calls", "[]", "--sleep", "-1"])) { error in
            XCTAssertEqual(
                error as? WandyCLIError,
                WandyCLIError(
    ...
    func testCLIRejectsUnknownOption() {
        XCTAssertThrowsError(try parseWandyCLI(arguments: ["--verbose"])) { error in
            XCTAssertEqual(
                error as? WandyCLIError,
                WandyCLIError(
    ...
    func testGeneralHelpListsCommandsAndGlobalFlags() {
        ...
        XCTAssertTrue(help.contains("wandy [command] [options]"))
    ...
    func testResolvedVersionFallsBackWhenBundleHasNoVersionMetadata() {
        XCTAssertEqual(resolvedWandyVersion(bundle: Bundle(for: Self.self)), openComputerUseVersion)
    }
    ...
    func testElementIndexAcceptsNumericCallSequenceArgument() throws {
        let calls = try readWandyCallSequence(
    ...
    func testReadToolArgumentsRejectsNonObject() {
        XCTAssertThrowsError(try readWandyToolArguments(json: #"["TextEdit"]"#, file: nil)) { error in
            XCTAssertEqual(
                error as? WandyCLIError,
                WandyCLIError(message: "--args must be a JSON object", helpCommand: "call")
            )
        }
    }
    ...
    func testMacOSAppAgentProxyDecisionKeepsNonAutomationCommandsLocal() {
        for command in [
            WandyCLICommand.turnEnded(payload: nil),
            .help(command: nil),
            .version,
        ] {
    ...
    func testMacOSAppAgentSocketPathIsScopedToUserAndAppBundle() {
        ...
        let stableAppURL = URL(fileURLWithPath: "/Applications/Wandy.app")
        let devAppURL = URL(fileURLWithPath: "/Users/me/Build/Wandy.app")
        ...
        XCTAssertTrue(stablePath.hasPrefix("/tmp/wandy-agent-501-"))
    ...
    func testPreferredPermissionAppBundleURLPrefersInstalledCopyOverTransientRunningCopy() {
        let installed = URL(fileURLWithPath: "/opt/homebrew/lib/node_modules/wandy/dist/Wandy.app")
        let running = URL(fileURLWithPath: "/Users/example/projects/wandy/dist/Wandy.app")
        let fallback = URL(fileURLWithPath: "/Users/example/projects/wandy-debug/dist/Wandy.app")
    ...
    func testPreferredPermissionAppBundleURLPrefersRunningDevelopmentCopy() {
        let installed = URL(fileURLWithPath: "/Applications/Wandy.app")
        let running = URL(fileURLWithPath: "/Users/example/projects/wandy/dist/Wandy (Dev).app")
        let fallback = URL(fileURLWithPath: "/Users/example/projects/wandy-debug/dist/Wandy (Dev).app")
    ...
    func testPreferredPermissionAppBundleURLCanPreferRunningReleaseCopyOverStaleInstalledCopy() {
        let staleInstalled = URL(fileURLWithPath: "/Users/example/projects/wandy/dist/npm/wandy/dist/Wandy.app")
        let running = URL(fileURLWithPath: "/opt/homebrew/lib/node_modules/wandy/dist/Wandy.app")
    ...
    func testPreferredInstalledAppBundleURLUsesFirstDiscoveredInstalledCopy() {
        let applications = URL(fileURLWithPath: "/Applications/Wandy.app")
        let npm = URL(fileURLWithPath: "/opt/homebrew/lib/node_modules/wandy/dist/Wandy.app")
        let duplicateApplications = URL(fileURLWithPath: "/Applications/Wandy.app")
    ...
    func testInitializeResponseContainsToolsCapability() throws {
        ...
        XCTAssertTrue(response!.contains(#""name":"wandy""#))
    ...
    func testVisualCursorEnvFlagDefaultsToEnabled() {
        ...
        XCTAssertTrue(visualCursorEnabled(environment: ["WANDY_VISUAL_CURSOR": "1"]))
        XCTAssertFalse(visualCursorEnabled(environment: ["WANDY_VISUAL_CURSOR": "0"]))
        XCTAssertFalse(visualCursorEnabled(environment: ["WANDY_VISUAL_CURSOR": "false"]))
    }

    func testInputFallbackDebugFlagDefaultsToDisabled() {
        XCTAssertFalse(inputFallbackDebugEnabled(environment: [:]))
        XCTAssertTrue(inputFallbackDebugEnabled(environment: ["WANDY_DEBUG_INPUT_FALLBACKS": "1"]))
        XCTAssertTrue(inputFallbackDebugEnabled(environment: ["WANDY_DEBUG_INPUT_FALLBACKS": "true"]))
        XCTAssertFalse(inputFallbackDebugEnabled(environment: ["WANDY_DEBUG_INPUT_FALLBACKS": "0"]))
        XCTAssertFalse(inputFallbackDebugEnabled(environment: ["WANDY_DEBUG_INPUT_FALLBACKS": "off"]))
    }

    func testGlobalPointerFallbackFlagDefaultsToDisabled() {
        XCTAssertFalse(globalPointerFallbacksEnabled(environment: [:]))
        XCTAssertTrue(globalPointerFallbacksEnabled(environment: ["WANDY_ALLOW_GLOBAL_POINTER_FALLBACKS": "1"]))
        XCTAssertTrue(globalPointerFallbacksEnabled(environment: ["WANDY_ALLOW_GLOBAL_POINTER_FALLBACKS": "yes"]))
        XCTAssertFalse(globalPointerFallbacksEnabled(environment: ["WANDY_ALLOW_GLOBAL_POINTER_FALLBACKS": "0"]))
        XCTAssertFalse(globalPointerFallbacksEnabled(environment: ["WANDY_ALLOW_GLOBAL_POINTER_FALLBACKS": "false"]))
    }
