...
public struct PermissionDiagnostics: Sendable {
    ...
    public var repairMessage: String {
        if allGranted {
            return "Wandy permissions are ready. \(summary)"
        }

        let statuses = SystemPermissionKind.allCases.map { permissionStatus($0) }
        if needsRuntimePermissionRefresh {
            return "Wandy is enabled or partly enabled in macOS Settings, but the running helper has not picked up the latest permission state. Click Restart in the Wandy permission window, then retry this request. \(summary)"
        }

        if statuses.contains(where: { $0.status == .unknown }) {
            return "Wandy could not read the macOS permission record, and the running helper is not authorized yet. Open the Wandy permission window, confirm Accessibility and Screen Recording, then restart the helper if prompted. \(summary)"
        }

        return "Wandy needs macOS app permissions. I opened the Wandy permission window; approve the missing items once, then retry this request. \(summary)"
    }
...
public enum PermissionSupport {
    public static let bundleDisplayName = "Wandy"
    public static let bundleIdentifier = "com.synara.wandy"
    public static let developmentBundleDisplayName = "Wandy (Dev)"
    public static let developmentBundleIdentifier = "com.synara.wandy.dev"
    ...
    private static let appVariantInfoKey = "WandyAppVariant"
    ...
    public static func isWandyBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
    ...
    private static func preferredInstalledAppBundleURL() -> URL? {
        ...
        if let stableAppDir = env["SYNARA_WANDY_STABLE_APP_DIR"]?.trimmingCharacters(in: .whitespacesAndNewlines),
        ...
        appendCandidate(
            URL(fileURLWithPath: NSHomeDirectory(), isDirectory: true)
                ...
                .appendingPathComponent("wandy-app", isDirectory: true)
                .appendingPathComponent(releaseAppBundleName, isDirectory: true)
        )
        ...
        for prefix in homebrewPrefixes() {
            appendCandidate(prefix
                ...
                .appendingPathComponent("wandy", isDirectory: true)
                .appendingPathComponent(releaseAppBundleName, isDirectory: true)
            )
