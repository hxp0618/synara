...
func inputFallbackDebugEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["WANDY_DEBUG_INPUT_FALLBACKS"]?
...
func globalPointerFallbacksEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["WANDY_ALLOW_GLOBAL_POINTER_FALLBACKS"]?
        .trimmingCharacters(in: .whitespacesAndNewlines)
        .lowercased()
    else {
        return false
...
public final class ComputerUseService {
    ...
    private func debugInputFallback(tool: String, targetDescription: String, snapshot: AppSnapshot) {
        ...
        let appReference = snapshot.app.bundleIdentifier ?? snapshot.app.name
        fputs(
            "[wandy] global pointer fallback tool=\(tool) app=\(appReference) target=\(targetDescription)\n",
            stderr
        )
    }
    ...
    private func debugClickDecision(_ message: String) {
        ...
        fputs("[wandy] click decision \(message)\n", stderr)
    }
        ...
        do {
            ...
            guard globalPointerFallbacksEnabled(environment: ProcessInfo.processInfo.environment) else {
                throw ComputerUseError.message(
                    "click could not be handled through accessibility, and global pointer fallback is disabled. Set WANDY_ALLOW_GLOBAL_POINTER_FALLBACKS=1 to allow physical-pointer fallback for this process."
                )
            }
        }
    }
