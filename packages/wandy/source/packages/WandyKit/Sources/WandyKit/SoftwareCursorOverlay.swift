...
func visualCursorEnabled(environment: [String: String]) -> Bool {
    guard let rawValue = environment["WANDY_VISUAL_CURSOR"]?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() else {
        return true
    }
...
public func visualCursorObservationFileURL(environment: [String: String]) -> URL? {
    guard
        let rawPath = environment["WANDY_VISUAL_CURSOR_OBSERVATION_FILE"]?
...
public let openComputerUseTurnEndedNotificationName = Notification.Name("com.synara.wandy.turn-ended")

public func postWandyTurnEndedNotification() {
...
@MainActor
public func resetWandyVisualCursor() {
