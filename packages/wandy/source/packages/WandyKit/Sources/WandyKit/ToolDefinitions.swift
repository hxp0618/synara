...
public enum ToolDefinitions {
    public static let all: [ToolDefinition] = [
        ...
        ToolDefinition(
            name: "run_sequence",
            description: "Run multiple Computer Use actions in one fast local sequence when the next steps are already known. Use get_app_state before the sequence, and call get_app_state again afterward only if you need fresh element indices or visual confirmation. This tool is part of plugin `Computer Use`.",
            annotations: defaultAnnotations(),
            inputSchema: objectSchema(
                properties: [
                    "calls": arrayProperty(
                        description: "Ordered calls to run locally. Each item accepts tool/name and args/arguments.",
                        items: objectSchema(
                            properties: [
                                "tool": stringProperty(description: "Tool name to run"),
                                "name": stringProperty(description: "Alias for tool"),
                                "args": objectProperty(description: "Tool arguments"),
                                "arguments": objectProperty(description: "Alias for args"),
                            ],
                            required: []
                        )
                    ),
                    "sleep": numberProperty(description: "Seconds to wait between successful calls. Defaults to 0.05."),
                ],
                required: ["calls"]
            )
        ),
...
private func arrayProperty(description: String, items: [String: Any]) -> [String: Any] {
    [
        "type": "array",
        "description": description,
        "items": items,
...
private func objectProperty(description: String) -> [String: Any] {
    [
        "type": "object",
        "description": description,
