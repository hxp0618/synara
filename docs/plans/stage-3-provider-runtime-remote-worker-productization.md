...
- **最近稳定检查点**：`14f7dd2d`（真实 Codex/Claude authorized external-SSH product/failure 四矩阵通过：Codex
  `22 pass + 1 unsupported` / `16 pass`，Claude `22 pass + 1 unsupported` / `16 pass`，四个不同 runtime
  identity、exact owned-runtime cleanup 与聚合 Secret scan 零 finding）；此前 `3c523417`/`b1c52bae` 已通过
  相同受控第三方 Key/Base URL/自定义模型 profile 的 Kubernetes/Docker 四矩阵。前序 deterministic rollout checkpoint
  为 `39b9b328`（one-control-plane/two-Worker owned disposable Kind 上的 clean-SHA
  ...
  overlap、`100/100` active/terminal release pin 与 Worker binding；Audit 读取 `229` 条/`2` 页，Outbox 通过
  `topicPrefix=worker.release.` 精确返回六条 published history。正式证据见
  `docs/reports/stage-3-worker-release-rollout-load-41683366.md`。Provider Host crash predecessor 为 `cfecba63`，
  network/container failure predecessors 为 `ab88798d`/`7684c6d8`，纯 load/admission 为 `e944b449`，Local
  ...
  owned Kind deterministic core fixture 已由 `b07e5bd9` 更新为 17/17）
- **工作区状态**：COMPLETE；运行时代码与生产验收边界固定为
  `8415efa15cebc48a23723dbdb147d3bafd7071bf`
- **发布文档**：
  ...
  `docs/reports/stage-3-kubernetes-structured-user-input-recovery-b07e5bd9.md`、
  ...
  `docs/reports/stage-3-saas-web-artifact-download-88f922ed.md`、
  `docs/reports/stage-3-saas-web-active-worker-replacement-multibrowser-82adfc3f.md`、
  `docs/reports/stage-3-saas-web-structured-user-input-multibrowser-807ffa8c.md`
...
### 1.0 最终 production closure（2026-07-24）

Stage 3 已按运行时 clean SHA `8415efa15cebc48a23723dbdb147d3bafd7071bf` 收口。操作人接受本机保留的
`synara-stage3-prod`（TLS Registry、三节点 Vault、Kyverno、MinIO/WORM）作为正式 production-profile
证据，并授权写入一个 365 天 COMPLIANCE Object Lock 版本。Registry、Vault/KMS/Kyverno、Vault restore、
SIEM/WORM、Local、SSH、Docker、Kubernetes 六 child aggregate 与 deterministic three-node immutable
rollout/promote/rollback 均已通过；最终 aggregate Secret scan 为零 finding，临时 acceptance 资源已按 owner
精确清理。完整证据、指标和已知限制统一记录在
`docs/reports/stage-3-production-release-8415efa1.md`。

本阶段正式使用第三方 API Key、可选 Base URL 和自定义模型。每个 Provider Adapter 仍须通过轻量契约、
...
- 第三方 API Key/Base URL/自定义模型注入能力与具体 endpoint 的产品能力分开验收。`6b71703f` 与
  `f958c1b2` 曾对配置 profile 的 Codex Approval 和 Claude HTTP `502` 边界 fail closed；这些报告保留为阻塞
  诊断。修正后，clean SHA `b1c52bae` 的 Docker aggregate 与 `3c523417` 的 disposable-Kind Kubernetes
  aggregate 均通过 Codex/Claude product + controlled-failure 四子门。两者只接受冻结的 Codex
  `terminal-large` 与 Claude `compact` explicit unsupported，使用同一 child-boundary/Catalog/Image consensus、
  精确 cleanup 和零 Secret finding，且不持久化 Key/Base URL 或 operator 环境变量名。完整通过证据见
...
- 操作人已授权一个外部 SSH 验收目标，认证信息保持在仓库外。Runner 与 `ssh_release_gate.py` 已增加显式
  ...
  ownership-marked runtime 的精确删除，绝不删除/重启主机或改写 `authorized_keys`。clean SHA `14f7dd2d`
  已通过真实 Codex/Claude product/failure 四矩阵；四个 child 的 external host、operator identity source 与
  Host Key source 均未持久化，聊天中的认证值也未进入命令、日志或报告。正式证据见
  `docs/reports/stage-3-real-provider-ssh-release-gate-14f7dd2d.md`。
...
- 生产并发不冻结为单一数字；由 Tenant quota、Worker 数/每 Worker slot、CPU/内存 requests/limits 和实际
  ...
  `deploy/worker/production-load-sla.json` 固定最短 `1800s`、Control Plane admission P95/P99 `1s/2s`、
...
2. Codex、Claude、Cursor、Antigravity、Grok、Kilo、OpenCode、Pi 都有经过验证的能力矩阵；
   未支持能力返回稳定、可解释的错误，不静默降级。Droid 继续明确保留在远程 Provider Host catalog 外，
   不能因本地 Provider 存在而被远程调度。
...
Provider Protocol/Capability、跨 Worker Interaction、Remote Workspace/Git、Worker Image/rollout、SaaS
Control Plane 权威和四 Target Acceptance 均已完成。最终 clean runtime SHA `8415efa1` 的 production
证据见 `docs/reports/stage-3-production-release-8415efa1.md`。Stage 4 接手多集群调度、容量、Warm Pool、
云厂商 CNI/Eviction、RWX cache 与跨 Region 灾难恢复；订阅/OAuth Provider 登录作为低优先级兼容项延期。
...
- Codex、Claude 远程能力补齐。
- Cursor、Antigravity、Grok、Kilo、OpenCode、Pi 的远程 Adapter 接入或明确阻断；Droid 明确保持
  Provider Host catalog 外的 Local-only 边界。
...
- Cursor、Antigravity、Grok、Kilo、OpenCode、Pi 先完成审计，再决定 Tier 1、Tier 2、Experimental
  或 Local-only。
...
- Clean commit `b07e5bd9` 将 owned Kind deterministic core suite 更新为 `17/17`，新增 Pending Structured User
  Input Pod-loss：同一 Turn/Execution 从 Generation `1 -> 2`，旧 request 精确变为 `expired/superseded`，新
  request 使用 `user-input.requested` Event 并保留完全一致的结构化问题，只允许 Generation 2 的
  `fixture-choice=Continue` 进入单终态。Approval 恢复保持原 API 依赖和行为；fixture Credential 同时迁移到
  产品支持的 `api_key` / `apiKey` 形态。报告见
  `docs/reports/stage-3-kubernetes-structured-user-input-recovery-b07e5bd9.md`。
- Clean commit `807ffa8c` 关闭真实 SaaS 两页面 Structured User Input 收敛/并发 Resolve：两个独立 Browser
  context 同时展示同一请求，不同答案竞争精确得到一个 `200` 与一个 `409 idempotency_conflict`，两页无刷新
  收敛到唯一答案；旧页面 auto-submit timer 发出 `0` 个过时请求，replacement request 不继承旧草稿，三个
  resolution 均 `acknowledged`，且只有一个 `execution.completed`。终态两页的 Session Event reconnect/error
  横幅均折叠。报告见
  `docs/reports/stage-3-saas-web-structured-user-input-multibrowser-807ffa8c.md`。
- D 尚未整体完成：仍需 Provider 不支持恢复时的跨 Target 故障矩阵，以及真实 Codex/Claude Adapter 的远程
  Interaction Release Acceptance。
...
- SecretGuard、agentd Credential/terminal tests、真实 PostgreSQL/SQLite migration tests、四 Target
  Acceptance 和 aggregate output scans 已通过；运行时证据的最终 Secret scan 为零 finding。Windows FD
  transport 不是本次 Linux/macOS production profile 的发布范围，保留为平台扩展项。
- Control Plane 对 managed Codex/Claude Credential 的新建和轮换实施 provider-specific 严格校验：只接受
  `apiKey` 与可选、无 userinfo/fragment 的绝对 HTTP(S) `baseUrl`。Web 使用 Codex/Claude 专用密码字段与
  Base URL 字段，其他 Provider 继续保留 advanced JSON；历史 Codex `organization` 与 Claude `authToken`
  仅在解密解析旧记录时兼容，不能由新建/轮换继续写入。Provider code 比较统一归一化，避免
  `claudeAgent` 的大小写表示导致可用 Credential 被前端遗漏。
- 共享 Acceptance Runner 的真实 SSH/Docker/Kubernetes 路径现要求显式指定 operator-owned 环境变量，
  ...
  环境变量名或 Secret。缺失、空值、控制字符和不安全变量名会在 CLI preflight 阶段 fail closed；Codex/
  Claude 的 `apiKey` 与可选 Base URL 均走同一受控路径。这是 Codex/Claude 第三方 API Key endpoint 的正式
  受控路径，不要求把第三方 Secret 或 endpoint 固化到代码、镜像或 Target 配置。该实现尚未生成真实
  Docker Provider 报告，因此不改变 G 的 `partial` 状态。
...
- Clean commit `d3af9380` 进一步从同一 clean SHA 构建并推送 baseline/candidate 两个不同 Registry Digest，
  ...
  在 baseline Approval Execution 持有 active Lease 时保持原容器与 Generation，canary 不占用该 Busy slot，
  candidate promote 精确返回 `409 worker_release_active_executions`；终态后旧容器才替换并继续收敛。第二个
  active-canary Execution 同样阻断 promote/rollback。四个 Execution 的 Container/Manifest/Revision/Channel/
  Digest 完全一致，Session Event 为连续 `1..33` 且无双执行或重复终态；`2` 条 Revision Audit、`4` 条
  Policy Audit 与 `6` 条已发布 Outbox 完整，精确 cleanup 和 Secret scan 通过。完整证据见
  `docs/reports/stage-3-worker-release-rollout-d3af9380.md`。
- Clean commit `41683366` 在同一 rollout gate 中保留原 busy baseline Approval，再启动 candidate/canary
  Approval 形成两 Worker/两 Revision overlap；精确删除 candidate container 后，同 Execution/logical Worker/
  Revision/Channel/Manifest/digest 保持，Generation `1 -> 2`，baseline peer 不变。恢复 pending 窗口内
  promote/rollback 均 fail closed。随后 candidate promoted `13` 波/`52` Execution 与 baseline rollback `12`
  波/`48` Execution 全部完成，共 `50/50` quota rejection/retry、`75/75` overlap、`100/100` active/terminal
  release pin 与 Worker binding。Audit cursor 分页和 Outbox `topicPrefix` 过滤保证发布历史在负载下仍可见。
  完整证据见 `docs/reports/stage-3-worker-release-rollout-load-41683366.md`。
- Clean commit `d1f3b68a` 在 one-control-plane/two-Worker owned Kind 上从同一 repository 真实拉取两个
  Registry digest，完成 `promote -> 100% canary -> promote -> rollback`。Busy baseline 与 candidate canary
  两个 execution-pinned Pod 同时 pending，Pod/Worker/Manifest/Revision/Channel/digest 完全一致；stale CAS、
  active baseline promote 和 active candidate rollback 均精确 fail closed。六 Session 均为 Event `1..12`、
...
- 未配置 Control Plane 时，上述门禁返回 local-allowed，现有本地 Provider Runtime 和 Native API 路径
  保持不变。
- Clean commit `3a6d347d` 在隔离 PostgreSQL/MinIO/Go Control Plane 与 Web 上创建真实 Project、Session、
  Turn 和 queued Execution；页面刷新与 Synara Server 重启后恢复同一 Session ID、Event Sequence
  `1:session.created,2:turn.created`，Browser Console 无相关 Error/Warning。该运行同时发现并修复同源 Proxy
  在下游 EventSource 断开后未取消上游 fetch 的 SSE lease 泄漏；clean-SHA 浏览器连续刷新始终保持单一
  lease，关闭页面后归零，独立三秒代理探针严格得到 `0 -> 1 -> 0`。该修复不新增 DDL，migration boundary
  仍为 `000041_diff_artifact_kind.sql`。完整证据见
  `docs/reports/stage-3-saas-web-control-plane-authority-sse-3a6d347d.md`。
- Clean commit `0b4d8e4e` 在完全未配置 Control Plane 的隔离 Web 上通过真实 Codex 两轮本地主聊天：首轮
  `LOCAL_MODE_OK` 后页面刷新保持同一 route/transcript，完整 Synara Server/dev restart 后仍从本地 SQLite
  恢复；第二轮 `LOCAL_MODE_RESUMED` 通过 Codex `thread/resume` 复用同一 Provider thread identity。Browser
  Console 全程无相关 Error/Warning，SQLite 严格为四条非流式 native 消息、`env_mode=local`、已有 resume
  cursor，且 `control_plane_named_tables=0`。该证据关闭 K 的 no-Control-Plane local main-chat regression
  gate，不扩张为本地 Project 文件操作、其他 Provider 或 SaaS Artifact/Worker 证明。完整证据见
  `docs/reports/stage-3-web-local-mode-restart-resume-0b4d8e4e.md`。
- Clean commit `88f922ed` 关闭 SaaS Web 的 Artifact Ready/list/download/recovery 子门禁：真实 PostgreSQL/MinIO
  Session 的 `artifact.ready` Sequence 驱动权威列表刷新，Web 只展示 Ready 的用户可下载 kind，并通过新鲜
  download grant 取得 `64` 字节 `text/plain` Payload；Browser Network 记录授权 `POST=200`、MinIO `GET=200`，
  独立 SHA-256 与数据库 Metadata 一致。页面刷新、SSE reconnect、完整 Server/dev restart 后仍恢复同一
  Session route、Artifact row 和下载动作，Console/框架 Overlay 为零，SSE lease 在关闭页面后回到零。该证据
  不扩张为兼容 Worker 实时输出、Provider-native Artifact、Approval/Input 或多浏览器证明。完整证据见
  `docs/reports/stage-3-saas-web-artifact-download-88f922ed.md`。
- `0eeabbc1` baseline 的 patched worktree 关闭 deterministic compatible-Worker Web projection/restart 子门禁：
  embedded Local agentd 的 compatible Codex fixture 在同一权威 Session 完成两轮；首轮投影 text、Tool、usage、
  Ready generated file 与 Checkpoint，Control Plane + embedded Worker 重启后 Worker incarnation `1 -> 2`，第二轮
  从首轮 Checkpoint 恢复 Workspace 并验证 generated file。Session Event `1..28` 连续且无重复，稳定新 Browser
  tab 的 Console 为零。运行同时修复 execution-scoped Runtime Event 缺少 payload `turnId` 时 Tool/usage/Artifact
  被过滤，以及 compatible Worker 返回后临时 `unobserved` Capability 缓存不自动收敛的问题；后者只在 Worker
  Manifest 未观测时每两秒重查，Supported 后停止。该证据不扩张为真实 Provider、active mid-Turn Worker
  replacement、Approval/Input 或多浏览器证明。完整证据见
  `docs/reports/stage-3-saas-web-compatible-worker-restart-0eeabbc1.md`。
- `82adfc3f` baseline 的 patched worktree 进一步关闭 deterministic active mid-Turn Worker replacement 与
  multi-browser 子门禁：同一 SaaS Session/Turn/Execution 在 pending Approval 窗口硬终止 standalone Local
  agentd，同一 logical Worker incarnation `1 -> 2`、Execution Generation `1 -> 2`，旧 request
  `expired/superseded`，新 request 只向 Generation `2` 交付并由原 Browser 解决；Session Event `12..34`
  连续且只有一个 `execution.completed`。Workspace/Materialization ID 不变，Generation `2` 从首轮 Ready
  Checkpoint 恢复并验证 generated file，随后生成新的 Ready Checkpoint。两个 Browser 页面又同时投影同一
  pending Approval，从第二页解决后两页无刷新收敛；并发模型切换精确得到一个 `200` 与一个
  `409 session_model_conflict`，冲突页重读权威 Session 后与胜出页收敛，未覆盖服务端模型。运行同时移除
  LegendList 与现有 imperative bottom-stick 的重复初始滚动所有权，并修复健康 SSE 存在时 REST catch-up
  不应留下 false `reconnecting` 状态的问题。该证据仍不扩张为真实 Provider、Structured User Input、远程
  Target、完整高级操作或生产 SLA/KMS 证明。完整证据见
  `docs/reports/stage-3-saas-web-active-worker-replacement-multibrowser-82adfc3f.md`。
- Clean commit `b07e5bd9` 补齐 Structured User Input 的 UI 与远程恢复边界：Browser component tests 覆盖
  单选延迟自动提交、多选不提前提交、replacement request remount 取消旧 timer，以及 resolving 时禁用选项；
  owned Kind `17/17` 又证明 execution-pinned Pod 丢失后旧 request `expired/superseded`、Generation `1 -> 2`、
  replacement 问题结构不变并只产生一个终态。该证据仍不替代真实 Provider live output/replacement 或完整高级
  操作。完整证据见
  `docs/reports/stage-3-kubernetes-structured-user-input-recovery-b07e5bd9.md`。
- Clean commit `807ffa8c` 进一步关闭真实 SaaS 双页面 Structured User Input 子门禁：竞争 Resolve 得到一个
  `200` 与一个 `409 idempotency_conflict`，两页无刷新收敛；过时单选 timer 请求为 `0`，replacement 不继承
  草稿，三个 resolution 全部 acknowledged，单 Execution 只有一个终态。运行同时将 catch-up 成功且 replacement
  SSE 已挂载的状态从 `reconnecting` 降为 `connecting`，两页终态 reconnect/error 横幅均折叠。该证据仍不替代
  真实 Provider、远程 Target 或完整高级操作。完整证据见
  `docs/reports/stage-3-saas-web-structured-user-input-multibrowser-807ffa8c.md`。
...
- Clean commit `b07e5bd9` 将 owned Kind core report 更新为 `17/17`：在既有 Pending Approval、Artifact、
  large Terminal、Provider Error、Control Plane Restart 和第二 Turn 基础上，新增 Pending Structured User
  Input 的 Pod Delete/Generation Fence/旧 request supersede/替换问题校验/单终态，并把 deterministic
  Credential 统一到产品支持的 `api_key` / `apiKey` 形态。完整证据见
  `docs/reports/stage-3-kubernetes-structured-user-input-recovery-b07e5bd9.md`。
...
- Clean commit `d1f3b68a` 已补齐 deterministic registry-pushed immutable Kubernetes Revision mechanics：同一
  repository 两个不同 digest 由 owned three-node Kind 通过 containerd mirror 与 `Always` 策略真实拉取，
  正式 API 完成 baseline promote、`100%` candidate canary、candidate promote 与 baseline rollback；四个
...
- SSH Driver 与 consolidated Gate 现同时支持显式 external-host：Host/user/port、owner-only 仓库外 identity、
  ...
  不记录 host、identity/Host Key source path 或 Provider Credential 环境变量名。该实现状态已由后续 clean
  `14f7dd2d` 的正式 external-host 四矩阵 supersede；证据见
  `docs/reports/stage-3-real-provider-ssh-release-gate-14f7dd2d.md`。
- Runner 已补齐真实远程 Provider 的受控认证入口：SSH/Docker/Kubernetes 不再允许依赖宿主机 ambient
  ...
  `/usr/local/bin/provider-host`，Credential 仍由 Control Plane 经 agentd FD 3 交付。受控 Key、Base URL 与
  自定义模型已由 clean `b1c52bae` Docker 和 `3c523417` disposable-Kind Kubernetes 四矩阵正式证明。
- SSH/Docker/Kubernetes 真实 Provider failure 路径已补齐受控 401/429 endpoint 与 scoped Host crash 注入。
  ...
  `/usr/local/bin/provider-host`，随后删除 machine/key 且未发现 Credential/private-key material。clean
  `14f7dd2d` 已在授权 external host 上通过真实 SSH 四矩阵；Docker/Kubernetes 受控 Provider profile 也已通过，
  但四 Target Gate 仍因 Local/Docker/Kubernetes 未在同一 release SHA 重跑而保持 open。
- Consolidated release gate 已抽取 target-aware 公共验证器，Local 既有 CLI/Schema 与冻结 Unsupported 边界
  ...
  ownership + ID 后自行删除。clean SHA `b1c52bae` 已通过真实 Docker 四矩阵：Codex product
  `22 pass + 1 unsupported`、failure `16 pass`；Claude product `22 pass + 1 unsupported`、failure `16 pass`。
  四个 child 共用同一 clean-SHA Image/Catalog，全部通过精确 cleanup 和 Secret scan；聚合扫描 `40` 个文件/
  `2,637,168` bytes 为零 finding。正式证据见
  `docs/reports/stage-3-real-provider-docker-release-gate-b1c52bae.md`；`f958c1b2` 保留为前序失败诊断。
- 受控远程 Gate 现进一步抽取 `controlled_remote_release_gate.py`，统一 Credential environment isolation、
  ...
  `29.4.0` runtime preflight 已通过。修正 `6b71703f` 暴露的真实阻塞后，clean SHA `3c523417` 使用
  operator-owned 第三方 Key/Base URL/自定义模型通过 Kubernetes 四矩阵：Codex product
  `22 pass + 1 unsupported`、failure `16 pass`；Claude product `22 pass + 1 unsupported`、failure `16 pass`。
  四个 disposable Kind cluster、共享 Image 与 child state 均精确清理，聚合扫描 `40` 个文件/
  `4,231,228` bytes 为零 finding。正式证据见
  `docs/reports/stage-3-real-provider-kubernetes-release-gate-3c523417.md`；`6b71703f` 保留为前序失败诊断。
...
- `ssh_release_gate.py` 的四个 child 各自 cross-build agentd/real Provider Host，并可选择唯一 disposable
  ...
  使用不同 Host Key。fixture、复用 installation/Host Key 或任一 runtime/cleanup mismatch 均 fail closed。clean
  `14f7dd2d` 随后使用 repository-external identity、单一 pinned Host Key 与授权 external host 通过真实 SSH
  四矩阵：Codex/Claude product 各 `22 pass + 1 unsupported`，failure 各 `16 pass`。四个 child 共享同一
  clean-SHA/Catalog、Provider Host SHA 与 Host Key fingerprint，并验证 `4/4` 不同 runtime identity、
  `leaseTTL=60s`、`heartbeatTimeout=180s`、external host preserved、owned runtime exact cleanup，以及 aggregate
  Secret scan `40` files / `3,177,374` bytes 零 finding；Host、identity/Host Key source path 与 Credential 环境变量名
  均未持久化。正式证据见 `docs/reports/stage-3-real-provider-ssh-release-gate-14f7dd2d.md`。该结果只关闭
  external-SSH product/failure slice，不关闭同一 release SHA 的四 Target 重跑、生产多节点/Registry/KMS/
  tlog/admission、并发/Retention/load 或 soak；没有 DDL 变更，migration boundary 仍为
...
- 新增 `docker_worker_release_rollout_gate.py`，从 clean commit `d3af9380` 构建并推送两个不同版本和
  Registry Digest，经正式 API 完成 immutable Revision、initial promote、canary、candidate promote 与
  baseline rollback。Gate 同时验证 Busy baseline Worker 在 active Lease 期间保持同一容器与 Generation、
  不占 canary slot、Target 保持 active、promote 精确阻断到终态，以及 stale CAS、active-canary fencing、
  Container/Manifest/Execution release pin、Transition/Audit/Outbox/Event 顺序、无双执行/重复终态、精确
...
- 新增并扩展 `fixture-load-failure`，复用同一 managed Docker 双 Worker、四 Session、quota/load、Provider
  ...
  标签的唯一容器。Clean commit `7684c6d8` 依次只中断 Worker 0 的 runner-owned network、再删除 Worker 1 的
  exact busy container；Docker reconciler 以同逻辑 Worker name/ID 重建新容器，container ID 与 instance UID
  变化、incarnation `1 -> 2`，named-volume sentinel 保留。两次故障都在两个 Session 同时 pending、另两个
  Session 无副作用 quota rejection 时证明 Peer Session 的 Event、Interaction、Worker 与 Generation 不变；
  受影响 Execution 保持同一 ID，将 Request/Interaction 替换并从 Generation 1 精确前进到 2，两边各一个
  terminal。随后同四 Session 完成 25 波次、`100/100` 唯一 Execution、`50/50` quota rejection/slot reuse、
  `75/75` overlap、Artifact Ready `200` 与 Checkpoint Ready `100`。正式 report `11/11`、Runner unit `134/134`、
  Stage 3 Python `246/246`、focused Go、`bun fmt`、`bun lint`（0 errors / 238 existing warnings）和
  `bun typecheck`（9/9）均通过；10 files / 2,733,690 bytes Secret scan finding 为零，精确 cleanup 后 owner
  资源零残留。该结果只关闭 deterministic single-host managed Docker network/container-loss failure targeting
...
- 该新增证据只关闭 deterministic single-host fixture Provider Host process crash classification and recovery
  ...
  `000041_diff_artifact_kind.sql`。
- Clean commit `41683366` 扩展 immutable Docker Worker Release gate，而不是创建第二套 load Driver。原 busy
  baseline 与 candidate/canary 两个 Approval 同时 pending，另两个 Session 无副作用 quota rejection；candidate
  exact container loss 后保持同 Execution/Worker/Revision/Channel/Manifest/digest 并从 Generation 1 前进到 2，
  baseline peer 全程不变，promote/rollback 在 recovered pending 窗口继续 fail closed。
- 随后 `25` 波正式 load 为 candidate promoted `52` + rollback baseline `48` 个 Execution，合计 `100/100`
  unique terminal、`50/50` quota rejection/retry、`75/75` overlap、`100/100` active release pin、`100/100`
  terminal release pin 与 `100/100` Worker binding。四 Session 总计 `102` 个唯一 Execution，Sequence 各自连续，
  无双执行/重复终态/Interaction 泄漏。
- Audit gate 改为有界 cursor pagination，正式读取 `229` 条/`2` 页；Outbox API 增加校验后的
  `topicPrefix`，正式精确返回六条 `worker.release.*` published history。Gate `15/15`、rollout unit `23/23`、
  Runner unit `136/136`、Stage 3 Python `253/253`、focused Go 与 Bun 三项通过；Secret scan 和精确 cleanup
  均通过。完整证据见 `docs/reports/stage-3-worker-release-rollout-load-41683366.md`。
- 该新增证据关闭 deterministic single-host release-rollout container-loss recovery/load/history mechanics；
  不关闭真实 Provider、已授权外部 SSH target/本机 `orbstack` context 的真实 gate、multi-host/Kubernetes
  multi-node、生产 Registry/KMS identity/tlog/admission、生产 SLA 或 production-duration load/soak。
  Workflow J/L 保持 `partial`，migration boundary 仍为 `000041_diff_artifact_kind.sql`。
...
- 当前进度：`000034`/`000037`、Worker/Releases API、Web 运维入口和 rollout Runbook 已形成；clean-SHA
  ...
  canary/promote/rollback 与 Busy Worker completion/fencing mechanics 已通过；clean commit `41683366` 进一步
  通过 candidate container-loss Generation `1 -> 2`、peer 隔离、`25` 波/`100` Execution release-pinned load、
  load-safe Audit/Outbox history 与精确 cleanup，正式 gate `15/15`。
  生产方向已选择 KMS；真实 KMS reference/signer identity/tlog/admission、生产 Registry
  Credential/retention、真实 Provider remote rollout、按资源档位验证的生产 SLA/soak 和生产多节点证据仍是
  完成门禁。
...
- 当前进度：clean commit `3a6d347d` 已通过隔离 Web 的真实 Control Plane Project/Session/Turn 创建、
  PostgreSQL 权威恢复、连续刷新、Browser reconnect、Server restart 与 SSE lease 精确回收；clean commit
  `0b4d8e4e` 又关闭无 Control Plane 的本地主聊天、刷新、Server restart 与 Codex native-resume 回退门禁。
  clean commit `88f922ed` 又关闭真实 SaaS Artifact Ready、列表、下载、刷新/reconnect/Server restart 恢复
  子门禁。`0eeabbc1` baseline 的 patched worktree 又关闭 deterministic compatible-Worker text/Tool/usage/
  Artifact/Checkpoint 投影与 Control Plane + embedded Worker between-Turn restart 恢复子门禁。真实 Provider、
  active mid-Turn Worker replacement、Approval/Input 和多浏览器并发仍是完成门禁。
...
- 当前进度：deterministic Local/Docker core、Local Provider fault、Docker network、Kubernetes
  ...
  最终 runtime SHA `8415efa1` 已由 Local、SSH、Docker、Kubernetes aggregate、production SLA 和
  production-duration soak 关闭；不得再按此历史 resume 位置重复执行已经通过的 Provider 矩阵。
...
- [x] 八个 Provider 均有审计完成的 Capability Matrix 和支持等级。
- [x] Tier 1/Tier 2/Experimental/Local-only 的发布边界明确。
- [x] Provider Host Protocol v2、最低兼容版本和 Capability Negotiation 冻结。
- [x] Unsupported Capability 返回稳定错误，不静默降级。
- [x] Start/Resume/Send/Steer/Interrupt/Compact/Rollback/Fork/Review 语义冻结并测试。
- [x] Approval、Structured User Input 和 Plan Mode 可以跨页面刷新、Control Plane 重启恢复。
- [x] Runtime Event 版本和未知事件策略完成。
- [x] Provider Resume Cursor 失效时可以按策略从权威历史继续；真实 Provider 的 Local/SSH/Docker/
      Kubernetes product/failure acceptance 与跨 Target 恢复证据已闭环。
- [x] Worker/Pod 替换后可继续后续 Turn，Event Sequence 连续。
- [x] Worker、Lease、Provider、Git Credential 没有非预期日志或 Artifact 泄漏。
- [x] Remote Workspace Clone/Fetch/Worktree/Checkpoint/Cleanup 生命周期完成。
- [x] Terminal、长日志、Generated File、Diff 和 Checkpoint 使用 Event/Artifact 引用。
- [x] Worker Drain、Graceful Shutdown、Manifest、升级、回滚和版本隔离完成。
- [x] Worker Image/Provider CLI 版本可追溯并可重复构建。
- [x] Web 主聊天在 SaaS 模式只使用 Go Control Plane 作为 Session 权威来源。
- [x] 未配置 Control Plane 时本地个人模式保持可用。
- [x] Local、SSH、Docker、Kubernetes 运行同一 Provider Acceptance Suite。
- [x] Crash、Network、Provider Failure、Pod Delete、Rolling Upgrade 测试通过。
- [x] 形成 Stage 3 Compatibility Matrix、Acceptance Report 和 Operations Runbook。
...
5. Cursor/Antigravity/Grok/Kilo/OpenCode/Pi 的远程支持结论和实现，以及 Droid 保持 catalog 外的明确边界。
