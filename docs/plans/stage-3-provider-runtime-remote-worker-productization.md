...
6. Web 普通聊天主路径是否完全以 Go Control Plane 为唯一 SaaS 权威来源仍需完成 Stage 2
   切换和验收。
7. 同源、可重复的 Provider Acceptance Suite 已覆盖 Local、SSH、Docker、Kubernetes；真实 SSH aggregate
   已在 clean `14f7dd2d` 通过，同一 release SHA 的四 Target 重跑与生产级跨 Target/规模证据尚未完成。
