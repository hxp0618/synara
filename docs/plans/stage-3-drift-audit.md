...
| L. Unified acceptance suite | partial | A shared Runner now emits machine-readable JSON, Markdown and redacted logs, drives Local, Docker and Kubernetes through user APIs and real Control Plane/agentd product paths, and models both standing and execution-pinned Worker allocation plus capability-declared managed replacement. The Kubernetes Driver provisions an isolated Namespace, ServiceAccount/RBAC, short-lived Token/CA and execution-pinned Pod/Manifest against a real disposable kind API; the deterministic Codex fixture passes the core continuity suite. Its default Provider remains a deterministic Protocol 2.1 fixture: this proves shared protocol and orchestration, not a real Codex/Claude release. SSH still fails with `runner.target_driver_missing`; Kubernetes Claude/failure cases, real Codex/Claude, SSH, long-session and the full failure matrix remain. |
...
6. In progress: the deterministic shared Runner covers Local, Docker and the Kubernetes core path and emits
   JSON/Markdown evidence. Implement the SSH Driver, finish Kubernetes Provider/failure coverage, and run real
   Codex/Claude adapters plus the long-session/failure matrix before promoting any Local-only Provider or claiming
   the four-Target release gate.
