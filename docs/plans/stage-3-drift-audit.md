...
| K. Web authority switch                       | partial              | SaaS Project/Session/Turn/Event is Control Plane authoritative and local mode remains isolated. Tenant switching clears the old Tenant query/subscription/draft scope, and SaaS Provider/advanced-operation handlers fail closed through Control Plane capability and Session projections without calling local Native API paths. Strict-CAS model-switch still reuses `000030`/`000031` state and adds no DDL for that local operation. Credential scope/binding administration uses `000033`, `000035`, `000036`, `000038` and `000039`; Worker management and release rollout use `000034`, `000037` and `000040`. Clean `3a6d347d` proves isolated Browser Project/Session/Turn authority, PostgreSQL restore, refresh/reconnect/Server restart and exact SSE lease cleanup (`0 -> 1 -> 0`) while fixing the same-origin proxy disconnect leak. Clean `0b4d8e4e` proves the no-Control-Plane local main-chat path: real Codex first Turn, page refresh, complete Server/dev restart, local SQLite restore and a second native-resume Turn on the same Provider thread, with zero Browser Console findings and no Control Plane-named local table. Clean `88f922ed` proves one Ready `generated_file` is reconciled from the durable `artifact.ready` Sequence into the SaaS chat, downloaded through a fresh grant with exact MinIO hash, and restored after refresh/reconnect/complete Server restart; Browser Console/overlay findings are zero and the SSE lease returns to zero. The `0eeabbc1` baseline patched worktree adds deterministic compatible-Worker Browser evidence: two text/Tool/usage Turns on one authoritative Session, one preserved generated file, Workspace restore from the first Checkpoint, Control Plane plus embedded Worker restart with Worker incarnation `1 -> 2`, contiguous Event Sequence `1..28`, and a clean stable Browser Console. It also correlates execution-scoped Runtime Events to their Turn and rechecks only temporary unobserved Capability projections until a compatible Worker returns. The `82adfc3f` baseline patched worktree adds active mid-Turn standalone Worker loss on one pending Approval: the same Session/Turn/Execution/Workspace advances Generation `1 -> 2`, expires and supersedes the stale request, exposes one replacement request to the live Browser, restores the previous Ready Checkpoint, verifies the generated file, creates a new Ready Checkpoint and emits one terminal Event. Two Browser pages also converge on one pending Approval and its remote resolution without reload; normal model switches propagate to the passive page, while a simultaneous model race returns one `200` and one strict-CAS `409 session_model_conflict`, after which both pages converge on the server winner. The same slice removes duplicate transcript initial-scroll ownership and prevents healthy SSE subscriptions from retaining a false reconnect state. Real Provider live output/replacement, Structured User Input, remaining advanced operations and remote-Target multi-browser evidence remain. |
| L. Unified acceptance suite                   | partial              | The shared Runner and target-specific Local, SSH, Docker, Kubernetes and Registry gates emit machine-readable reports with source consensus, cleanup and Secret scanning. Clean commit `253052aa` passes the real Codex/Claude Local product/failure aggregate. Clean commit `7659dd5f` passes the dual-platform Registry signing-policy gate, `d3af9380` passes the deterministic managed Docker immutable rollout Busy Worker gate `15/15`, `6e866a30` passes deterministic Local `100/100` Turn long-Session mechanics, `eeb7a2f1` passes managed Docker multi-Provider overlap `9/9`, `c27914da` passes Local Retention/Cleanup `9/9`, `e944b449` passes managed Docker `100`-Execution bounded load `9/9`, and `e2d70fb6` passes the same Docker path with a `300s` minimum duration, exact resource profile, `224/224` Executions and P50/P95/P99/error measurements. `ab88798d` and `7684c6d8` pass exact network/container recovery under load, and `cfecba63` passes exact fixture Provider Host crash classification and retry under load `12/12`. Clean commit `41683366` passes deterministic immutable release-rollout candidate container loss under promoted/canary overlap plus `25` release-pinned waves: `100/100` load Executions, `50/50` quota rejection/retry, `75/75` overlap, `100/100` active and terminal release-pin checks, paginated `229`-entry Audit history and topic-filtered six-message Outbox, all in `15/15`. Clean commit `aa1d0225` passes the complete deterministic three-node owned-Kind matrix `24/24`, including PDB blocking and cross-Worker recovery. Current Acceptance Runner unit tests are `164/164` and Stage 3 Python is `298/298`. Clean `6b71703f` executes the real Kubernetes four-child gate and clean `f958c1b2` executes the real Docker four-child gate: Codex controlled failure passes in both, while the configured Codex product and Claude profiles fail closed with preserved cleanup/Secret evidence. A real SSH aggregate, passing Docker/Kubernetes Provider profiles, approved production duration/P95/P99/error/recovery thresholds, concrete production KMS identity/tlog/admission, production Kubernetes multi-node rollout and real-Provider concurrency/Retention/load/failure remain open.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
6. In progress: the deterministic shared Runner covers Local, Docker, SSH and Kubernetes and emits JSON/Markdown
   ...
   pagination and Outbox topic filtering.
