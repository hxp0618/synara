...
Baseline: current `codex/saas-tenancy-user` worktree after `9f93c33a`, including the Cursor-lineage and shared
Acceptance Runner slice under verification on 2026-07-14.
...
| D. Approval and Structured User Input | partial | Migration `000019`, Worker APIs and agentd implement Generation-fenced pull/delivered/acknowledged transitions and obsolete-Generation superseding. Migration `000021` persists immutable Turn runtime/interaction modes through Web, Session Event and Worker Workload. Migration `000028` records the request Runtime Event version so canonical resolutions never guess from an ambiguous name. Real Codex and Claude Approval plus Plan Mode User Input pass Host round trips. Web still lacks the durable Interaction list/reconcile and authoritative resolve adapter required for refresh/restart completion. Claude uses a host-owned PreToolUse hook so local permission allow rules cannot bypass the durable decision. |
| E. Runtime Event compatibility | implemented baseline | `ProviderRuntimeEventV2` is now the only v2 wire vocabulary. Provider Host maps its bounded internal v1 messages to canonical v2 frames; agentd negotiates and enforces version 2; Control Plane keeps explicit legacy v1 while validating canonical v2 type/payload/size; Web projects both legacy and canonical Delta without duplicate Sequence application. Unknown Provider-native messages degrade to bounded `runtime.warning`; an unknown v2 wire type is rejected rather than silently reinterpreted. Eight-Provider golden fixtures and full Target acceptance remain under workflow L. |
| F. Authoritative history and Worker migration | partial | Migration `000018` and Session/Execution code create versioned Runtime Bindings; `000030` freezes the Credential version, Resume strategy and Provider binding digest per Execution generation. Cursor Envelope v2 authenticates that binding. Migration `000031` adds `absent / usable / quarantined`, source Execution/Generation/History Sequence lineage, legacy-ciphertext quarantine and the single-active-Execution index. Wrong-key, missing-Cipher, unknown/legacy-envelope and non-native Runtime cases preserve but quarantine ciphertext and fall back to authoritative history; explicit binding drift may discard it, and only a fresh Cursor from the current Execution restores `usable`. Cursor expiry policy, explicit fallback outcome Events and full real-target migration evidence remain. |
...
| K. Web authority switch | partial | SaaS Project/Session/Turn/Event is Control Plane authoritative and local mode remains isolated. Tenant switching now cancels/removes the old Tenant query scope, closes old Event subscriptions, clears cross-Tenant drafts and serializes rapid switches. Canonical Runtime Events project through the same Thread model. Durable Interaction list/resolve, Artifact Ready and Capability Unsupported still need first-class Web projection without restoring dual authority. |
| L. Unified acceptance suite | partial | A shared Runner now emits machine-readable JSON, Markdown and redacted logs, drives Local, Docker, SSH and Kubernetes through user APIs and real Control Plane/agentd product paths, and models both standing and execution-pinned Worker allocation plus capability-declared managed replacement. On 2026-07-14 the deterministic Codex fixture passed all 13 SSH cases on an isolated disposable OrbStack Ubuntu 24.04 VM: one-time credentials, pinned-Host-Key rejection, product install/upgrade/revoke, sshd restart, systemd Worker incarnation replacement, Workspace continuity, Control Plane restart, second-Turn continuity and exact cleanup. The Kubernetes Driver provisions an isolated Namespace, ServiceAccount/RBAC, short-lived Token/CA and execution-pinned Pod/Manifest against a real disposable kind API; the deterministic Codex fixture passes the core continuity suite. These fixture runs prove shared protocol and orchestration, not a real Codex/Claude release. Kubernetes Claude/failure cases, real Codex/Claude SSH and other Target runs, long-session and the full failure matrix remain. |
...
applied. Runtime `AutoMigrate` and hand-applied database mutation remain non-authoritative.
...
6. In progress: the deterministic shared Runner covers Local, Docker and the Kubernetes core path and emits
   JSON/Markdown evidence. The SSH Driver owns a disposable OrbStack VM, one-time credential, host-key pinning and
   product install/upgrade/revoke lifecycle; its deterministic Codex fixture passed the 13-case live SSH suite on
   2026-07-14. Finish Kubernetes Provider/failure coverage and run real Codex/Claude adapters across the required
   Targets plus the long-session/failure matrix before promoting any Local-only Provider or claiming the four-Target
   release gate.
