...
| C. Unified Session/Turn semantics | partial | Start/Resume/Send use correlated v2 Command IDs with in-process terminal replay and persisted Runtime Binding identity. Codex App Server and Claude Agent SDK pass native Cursor resume, authoritative-history fallback and native Turn interrupt. Migration `000022` adds a reusable Generation-fenced Control Command channel; Web interrupt intent, Worker delivery, Cursor-preserving acknowledgement and `execution.interrupted` projection are implemented. Steer, Compact, Rollback, Fork and Review still need remote implementations and broader cross-Worker tests. |
...
3. In progress: Codex App Server and Claude Agent SDK multi-Turn, native Interrupt, Approval, Plan Mode Input and history fallback are implemented; close cross-Worker/restart acceptance next.
