...
| L. Unified acceptance suite | partial | A shared Runner now emits machine-readable JSON, Markdown and redacted logs, drives Local, Docker, SSH and Kubernetes through user APIs and real Control Plane/agentd product paths, and models both standing and execution-pinned Worker allocation plus capability-declared managed replacement. The SSH Driver owns a disposable OrbStack VM, one-time SSH credential, host-key pinning and product install/upgrade/revoke lifecycle; live SSH evidence is not yet recorded. The Kubernetes Driver provisions an isolated Namespace, ServiceAccount/RBAC, short-lived Token/CA and execution-pinned Pod/Manifest against a real disposable kind API; the deterministic Codex fixture passes the core continuity suite. Its default Provider remains a deterministic Protocol 2.1 fixture: this proves shared protocol and orchestration, not a real Codex/Claude release. Kubernetes Claude/failure cases, real Codex/Claude, SSH live acceptance, long-session and the full failure matrix remain. |
...
6. In progress: the deterministic shared Runner covers Local, Docker and the Kubernetes core path and emits
   JSON/Markdown evidence. The SSH Driver now owns a disposable OrbStack VM, one-time credential, host-key pinning
   and product install/upgrade/revoke lifecycle; run its live acceptance, finish Kubernetes Provider/failure
   coverage, and run real Codex/Claude adapters plus the long-session/failure matrix before promoting any Local-only
   Provider or claiming the four-Target release gate.
