...
| H. Remote Workspace/Git lifecycle | partial | Migration `000020` creates one logical Workspace per Session and binds it to Target/Execution without persisting Worker paths or Credentials. Agentd now uses the logical Workspace ID for a stable Session checkout, renews the Lease during preparation, validates public HTTPS Repository URLs and branches, rejects embedded Credentials and local/private/link-local targets, DNS-pins Clone/Fetch with redirects disabled, creates an isolated Session branch, and reports `workspace.ready/failed` through Generation-fenced idempotent Worker APIs before Provider start. Private Git Credential/SSH delivery, shared cache plus `git worktree`, Checkpoint restore, dirty tracking and cleanup remain. |
...
4. In progress: Workspace/Git/Checkpoint DDL plus public-HTTPS Clone/Fetch and state reporting are implemented; add Git Credential, worktree/cache, Checkpoint restore and cleanup lifecycle.
