...
`provider_unavailable`; all clusters/image/state were removed and the aggregate Secret scan was empty. The
real-Provider gate therefore remains open rather than silently degrading the profiles. Production concurrency is
governed by quota, Worker slots and CPU/memory resource profiles rather than one hard-coded number; production
signing will use `kms-key`, potentially through a self-hosted Vault `hashivault://...`
...
- 该证据关闭 deterministic single-host immutable release-rollout container-loss recovery under bounded load、
  ...
  没有 DDL 变更，migration boundary 仍为 `000041_diff_artifact_kind.sql`。当前 Acceptance Runner `156/156`、
  SSH Gate `15/15`、Stage 3 Python `278/278` 与 Control Plane `go test ./...` 均通过。
...
6. In progress: the deterministic shared Runner covers Local, Docker, SSH and Kubernetes and emits JSON/Markdown
   ...
   Clean commit `41683366` adds deterministic immutable release-rollout failure under load, load-safe Audit
   pagination and Outbox topic filtering.
   Run both real adapters across SSH, Docker and Kubernetes, then complete real Provider/remote Target
