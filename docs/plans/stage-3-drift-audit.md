...
Baseline: `codex/saas-tenancy-user` after clean commit `ac1e3e85`, including the deterministic managed Docker
immutable Worker Release rollout gate. The consolidated real Codex/Claude Local product and controlled-failure
...
summarized in `docs/reports/stage-3-worker-release-rollout-ac1e3e85.md`; the real Local release evidence remains in
...
| J. Worker drain/upgrade/version isolation     | partial              | Migrations `000017`, `000034`, `000037` and `000040` provide immutable Worker manifests, operator-revocation fencing, target-scoped Release Revisions, strict-CAS promoted/canary policy, transition history and release-pinned scheduling. Agentd Drain preserves the Workspace lock and reports conservative data-loss risk. Clean commit `7659dd5f` proves cached/no-cache multi-arch Registry reproducibility, SPDX/SLSA, embedded identity, default ephemeral exact-digest signing, `HIGH/CRITICAL=0`, Secret=0, EOSL/DB freshness and exact cleanup. Clean commit `ac1e3e85` proves deterministic managed Docker immutable Revision canary/promote/rollback, stale-CAS and active-Execution fencing, release-pin consistency, durable history and exact cleanup. Checked-in keyless/KMS production signing paths are implemented; real production signer identity/tlog/admission, Registry Credential/retention, real Provider remote rollout, multi-node canary/rollback/eviction and load evidence remain.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
> canary/rollback, and soak remain open. The deterministic managed Docker immutable rollout slice is closed by
> `ac1e3e85`.
...
- 新增正式 `docker_worker_release_rollout_gate.py`。Clean commit `ac1e3e85` 从同一源码构建并推送
  `0.5.4+rollout.baseline` 与 `0.5.4+rollout.candidate` 两个不同 Registry Digest，创建 immutable Revision
  `5fcfc384-...` 与 `dedfcf59-...`，并经正式 API 完成 Policy `1 promote -> 2 canary -> 3 promote -> 4 rollback`。
- stale `expectedPolicyVersion` 返回 `409 worker_release_policy_version_conflict`；candidate canary 上存在 active
  Approval Execution 时，promote 与 rollback 均返回 `409 worker_release_active_executions`。Approval resolve
  后 Execution 恰好一个 terminal，随后 candidate promote 与 baseline rollback 成功。
- 每个阶段的 Container、Worker Manifest、Execution Revision/Channel 与 Registry Digest 完全一致；三个
  Execution 的 Session Event 形成连续 Sequence `1..26`，无双执行、无重复终态。Audit 保留 `2` 条 Revision
  与 `4` 条 Policy entry，Outbox `6` 条全部 published，Transition 与最终 Policy 一致。
- 正式 gate `14/14`、全部 release-gate tests `105/105`、Stage 3 Python `208/208`；Go Worker Release focused、
  Web unit `44/44` 与 browser `3/3` 通过。Cleanup 精确删除三个 Worker container、Registry、三个 volume、
  network、两个 Worker image slot 与 state，`broadCleanupUsed=false`；Secret scan 覆盖 17 files / 323,825 bytes，
  finding 为零。完整证据见 `docs/reports/stage-3-worker-release-rollout-ac1e3e85.md`。
- 该结果关闭 deterministic managed Docker immutable rollout mechanics，不关闭生产 Registry TLS/auth/retention、
  真实 keyless/KMS identity/tlog/admission、真实 Codex/Claude Docker/SSH/Kubernetes Credential gate、Kubernetes
  多节点 rollout、Busy Worker 长任务、load 或 soak。Workflow J/L 因此继续保持 `partial`。本切片没有 DDL
