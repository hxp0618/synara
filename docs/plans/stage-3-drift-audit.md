...
| C. Unified Session/Turn semantics | partial | Start/Resume/Send use correlated v2 Command IDs with in-process terminal replay and persisted Runtime Binding identity. Codex now uses native App Server Thread resume and Turn interrupt; Claude retains tested emulated process termination. The Control Plane interrupt intent path plus Steer, Compact, Rollback, Fork and Review still need remote implementations and cross-Worker tests. |
| D. Approval and Structured User Input | partial | Migration `000019`, Worker APIs and agentd implement Generation-fenced pull/delivered/acknowledged transitions and obsolete-Generation superseding. Migration `000021` persists immutable Turn runtime/interaction modes through Web, Session Event and Worker Workload. Real Codex App Server Approval and Plan Mode User Input pass Host round trips; Claude remains explicitly Unsupported, and refresh/restart Web acceptance remains. |
...
3. In progress: Codex App Server multi-Turn, native Interrupt, Approval, Plan Mode Input and history fallback are implemented; close Claude Agent SDK and cross-Worker/restart acceptance next.
