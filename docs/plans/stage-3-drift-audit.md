...
| J. Worker drain/upgrade/version isolation | partial | Migration `000017` stores immutable Worker/Image/Provider manifests; registration deduplicates by Hash, records all eight Provider conclusions, and Claim binds/rejects by compatibility. The root Dockerfile now pins the Dockerfile frontend and all Worker Base Image digests, installs Codex/Claude CLIs from an npm lock with integrity hashes, pins the Alpine package closure, and generates a normalized SPDX document plus `/opt/synara/worker-image-manifest.json`. Agentd strictly validates the manifest schema, platform, referenced Lock/SBOM hashes, full source Git SHA and three separate Codex CLI/Claude SDK/Claude CLI versions before registration, then publishes only a path-free `featureFlags.workerImageBuild` summary. Docker/Kubernetes no longer overwrite the image version with `managed`; a digest-qualified deployment reference supplies the final Image Digest without creating a self-reference. The official Buildx entrypoint passes Version/SHA/`SOURCE_DATE_EPOCH`, retains metadata and enables registry SBOM/Provenance attestations on `--push`; a local arm64 dirty-worktree verification image passed non-root, CLI, lock/SBOM hash and fail-closed identity checks, but it is not release evidence for the declared Git SHA. SIGTERM now sends an immediate Draining Heartbeat, stops Claim, keeps Heartbeat/Lease renewal through a bounded deadline, completes safe work or cancels and releases the Lease; stale Draining Workers become Offline. Legacy Runner cancellation and Provider Host abort terminate same-group Unix descendants and a Windows kill-on-close Job Object before the Workspace lock is released; command writes and durable-control terminal waits now honor cancellation. Deliberate Unix process-group escape, Windows FD3 Credential delivery, a clean registry-pushed multi-arch reproduction run, canary/rollback and incompatible/revoked operations remain. Managed SSH/Docker/Kubernetes carry a 20-second deadline inside their 30-second stop window, and embedded Local drain finishes before HTTP shutdown. |
| K. Web authority switch | partial | SaaS Project/Session/Turn/Event is Control Plane authoritative and local mode remains isolated. Tenant switching cancels/removes the old Tenant query scope, closes old Event subscriptions, clears cross-Tenant drafts and serializes rapid switches. Canonical Runtime Events and durable pending Interaction list/resolve project through the same Control Plane-backed Thread model. SaaS Provider selection and sends fail closed through the Control Plane capability projection; local-only/Droid are disabled before creation, unknown Providers fail closed, and SaaS handlers do not call local Compact/Review/Fork/Side or Provider-native command paths. Existing SaaS Session model switching is now persisted by a strict-CAS Control Plane action that rejects Active Executions and unobserved/unsupported capability, clears the old Cursor, releases and revisions the Runtime Binding, emits `session.model.changed`, and updates the Web projection only after success; Codex/Claude advertise this behavior as `emulated`. No new DDL is required beyond Migration `000031`. Artifact Ready plus explicit browser evidence for refresh/reconnect/Server restart and no-Control-Plane local mode remain. |
...
The Provider capability projection increment reuses existing Execution Target, Worker/Provider Manifest, Session and
Execution state. The migration boundary remains `000031`; this increment adds or modifies no DDL.
