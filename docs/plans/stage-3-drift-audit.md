...
Baseline: current `codex/saas-tenancy-user` worktree after `1e598cda`, including the Runtime Event v2 and
Interaction-version migration slice under verification on 2026-07-13.
...
| --- | --- | --- | --- |
| Codex | App Server adapter with Send, Steer, Review, Interrupt, Approval/Input, Compact, Rollback and Fork paths | Bidirectional Codex App Server v2 runtime with native Cursor, Interrupt, Steer, Approval/Input and bounded history reconstruction | Tier 1 target |
| Claude | Agent SDK adapter with multi-turn, Interrupt, Approval/Input, history and discovery paths | Streaming Claude Agent SDK v2 runtime with native Session ID, Interrupt, Steer, Approval/Input and bounded history reconstruction | Tier 1 target |
...
3. In progress: Codex App Server and Claude Agent SDK multi-Turn, native Interrupt/Steer, Approval, Plan Mode Input and history fallback are implemented. Runtime Event v2 is canonical end to end. PostgreSQL cross-Worker Turn/Cursor/history/sequence continuity is covered; close Web Interaction authority and real Target restart acceptance next.
