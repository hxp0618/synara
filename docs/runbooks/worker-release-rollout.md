...
Busy baseline Worker 容器/Generation 保持、candidate container-loss Generation 恢复、四 Session bounded
release-pinned load、分页 Audit、topic-filtered Outbox、Event Sequence 与精确 cleanup。它不使用生产 Registry
Credential/TLS，也不执行真实 Provider、Kubernetes 多节点、生产 SLA 或 production-duration soak，因此不能
替代下面的生产预检与观察窗口。

同一 clean commit 还应运行 execution-pinned Kubernetes rollout mechanics gate：
...
单独审批并写入非 Secret policy；不得把尚未确定的自建 KMS 当作已完成生产签名证据。
