...
4. Canary 完成真实 Provider Describe、Start、Send、第二 Turn、Interaction、Artifact 和错误分类。
5. Worker replacement/Drain 没有重复终态、双 Worker 写入或 Generation 回退。
6. Control Plane 5xx、Execution failure/recovery、Worker offline、Artifact failure 和 Secret scan 无异常。
7. Docker/Kubernetes 实际运行镜像的 registry-returned Digest 与 Revision Manifest 一致。

当前 deterministic image-canary 只使用同内容 alias，不能满足第 4、7 项，也不能替代正式 rollout。
...
Promote 后继续观察旧 Worker graceful Drain、Busy Execution 终态和新 Worker Claim，不要立即清理旧镜像。
