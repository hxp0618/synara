...
| Kubernetes fixture                                     | `fc9b2bf6` owned Kind 23/23；`6b71703f` OrbStack 22 pass/1 unsupported | owned Kind Drain/uncordon、Eviction、Network、Canary、restart 与 exact cleanup 已验；不证明 PDB、多节点或真实 Provider pass gate                                         |
...
- [x] owned disposable Kind 已完成 deterministic clean-SHA `23/23`，包括 exact Node Drain/uncordon、Generation
      `1 -> 2` fencing、`policy/v1` Eviction、Canary、restart 与精确 cluster/image cleanup；证据为
      `docs/reports/stage-3-kubernetes-kind-drain-fixture-fc9b2bf6.md`。PDB 与 multi-node 仍未关闭。
