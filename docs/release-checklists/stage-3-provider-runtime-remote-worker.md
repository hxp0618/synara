...
| Kubernetes fixture                                     | `1e826324` OrbStack 19/19；`2763ebd3` Kind 13/13 | clean-SHA approved Context、Eviction、Network、Canary、restart 与 exact cleanup 已验；仍不是真实 Provider gate                                                           |
...
- [x] 本地 `orbstack` context 已完成 deterministic clean-SHA `19/19`、Context/TLS pinning、共享本地镜像
      `Never` 策略与精确 cleanup；证据为
      `docs/reports/stage-3-kubernetes-orbstack-fixture-1e826324.md`。
- [ ] 已授权外部 SSH target 尚需 external-host 模式和 clean-SHA 运行；`orbstack` 仍需真实 Codex/Claude
      Credential release gate。SSH 认证信息不得写入仓库或 evidence。
