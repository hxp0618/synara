...
- `python3 deploy/kubernetes/validate-resilience-assets.py`: PASS.
- `kubectl --context orbstack apply --dry-run=client` for the example ConfigMap and Deployment: PASS.
- `docker compose -f deploy/saas/docker-compose.yml config --quiet` with ephemeral required values: PASS.
- `git diff --check`: PASS.
